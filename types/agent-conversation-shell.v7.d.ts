...
import type { ApprovalAgentBinding, ApprovalReason } from './approval.v2.js'
...
type AgentConversationPendingApprovalActions =
  readonly [
    AgentConversationApprovalAction & { decision: 'approve' },
    AgentConversationApprovalAction & { decision: 'reject' },
  ]
export interface AgentConversationApprovalAuthority {
...
type AgentConversationApprovalItemBase = {
  ...
  requester: AgentDefinitionIdentity
  authority: AgentConversationApprovalAuthority
  reason: ApprovalReason
}
export type AgentConversationApprovalItem =
  | (AgentConversationApprovalItemBase & { state: 'pending'; agentGeneration: number; authorityBinding: ApprovalAgentBinding; actions: AgentConversationPendingApprovalActions; diagnostic?: never })
  | (AgentConversationApprovalItemBase & { state: 'approved' | 'denied' | 'cancelled'; agentGeneration?: number; authorityBinding?: never; actions: readonly []; diagnostic?: never })
  | (AgentConversationApprovalItemBase & { state: 'failed'; agentGeneration?: number; authorityBinding?: never; actions: readonly []; diagnostic: LocalizedText })
...
export type AgentConversationShellCommandContext =
  | { binding: { bindingId: string; ownerGeneration: string }; generation: string; scope: 'header'; command: CommandReference; itemId?: never; approval?: never; submitPayload?: never }
  | { binding: { bindingId: string; ownerGeneration: string }; generation: string; scope: 'message'; itemId: string; command: CommandReference; approval?: never; submitPayload?: never }
  | { binding: { bindingId: string; ownerGeneration: string }; generation: string; scope: 'approval'; itemId: string; command: CommandReference; approval: { sessionId: SessionId; approvalId: string; requester: AgentDefinitionIdentity; authority: ApprovalAgentBinding; decision: 'approve' | 'reject' }; submitPayload?: never }
  | { binding: { bindingId: string; ownerGeneration: string }; generation: string; scope: 'composer-submit'; command: CommandReference; submitPayload: string; itemId?: never; approval?: never }
