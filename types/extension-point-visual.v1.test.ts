...
const manifest: PluginRuntimeManifestV10 = {
  ...
  capabilities: [
    ...
    { name: 'ui.extension-points.interact', required: false, scope: { extensionPoints: ['composer.frame.overlay'], events: ['pointer.observe'] } },
  ],
  services: [],
}
const snapshot: ExtensionPointVisualSnapshotV1 = {
  schemaVersion: 1, sequence: 1, action: 'voice', enabled: true, busy: false,
  draftEmpty: true, accessibleLabel: 'Start voice chat', theme: 'dark', reducedMotion: false,
  bounds: { width: 28, height: 28 }, pointer: null, events: [],
}
void manifest
void snapshot
