...
const approvalCommandContext = {
  ...
  command: { id: 'chatroom.approval.approve' },
} satisfies import('@cordisx/protocol/agent-conversation-shell/v3').AgentConversationShellCommandContext
approvalCommandContext.scope satisfies 'approval'
const selfIntroductionAuthor = {
  participantId: 'agent-1',
  role: 'agent',
  displayName: { key: 'agent.one', fallback: 'Reviewer' },
  agentIdentity: { agentId: 'reviewer', revision: 'definition-1' },
} as const
const selfIntroductionMessage: AgentConversationMessageItem = {
  ...
  semantic: {
    ...
    causation: { operationId: 'operation-introduction-1' },
    participantId: selfIntroductionAuthor.participantId,
    memberId: 'member-1',
    runId: 'run-1',
...
const conversationMessage: AgentConversationMessageItem = {
  ...selfIntroductionMessage,
  messageId: 'message-conversation-1',
  semantic: { purpose: 'conversation' },
}
const acknowledgementMessage: AgentConversationMessageItem = {
  ...conversationMessage,
...
const failedApproval: AgentConversationApprovalItem = {
  ...pendingApproval,
  state: 'failed',
  actions: [],
  diagnostic: { key: 'approval.failed', fallback: 'Decision failed' },
}
void failedApproval

const emptyDescription: AgentConversationRoomDescription = { state: 'empty' }
const presentDescription: AgentConversationRoomDescription = {
  state: 'present',
  text: { key: 'room.description', fallback: 'A room for protocol work' },
}
...
if (roomCompositeVisual.kind === 'room-composite-avatar') {
  roomCompositeVisual.participants[0]?.avatar.kind satisfies
    | 'generated'
    | 'asset'
    | 'definition'
    | 'platform'
...
if (selection.kind === 'room' && selection.description?.state === 'present') {
  selection.description.text.fallback satisfies string
}
...
const replaceDescriptionPatch: AgentConversationRoomSettingsPatch = {
  name: 'Updated room',
  description: { state: 'present', text: 'New introduction' },
}
void namePatch
...
void emptyPatch
const localizedMutationDescription: AgentConversationRoomSettingsPatch = {
  // @ts-expect-error mutation description present state requires plain text
  description: { state: 'present', text: { key: 'room.description', fallback: 'Description' } },
}
...
void storagePatch
const rawAvatarVisual: AgentConversationRoomCollectionLeadingVisual = {
  kind: 'room-composite-avatar',
  roomId: 'room-1',
  // @ts-expect-error collection participant visuals require a formal AgentAvatarRef
  participants: [{ participantId: 'agent-1', avatar: '/avatars/agent.png' }],
}
void rawAvatarVisual
const callbackVisual: AgentConversationRoomCollectionLeadingVisual = {
  kind: 'semantic-icon',
...
const callbackApproval: AgentConversationApprovalItem = {
  ...pendingApproval,
  // @ts-expect-error approval actions are structured commands, never callbacks
  actions: [{ decision: 'approve', command: { id: 'chatroom.approval.approve' }, callback: () => undefined }],
}
...
const mismatchedAcknowledgement: AgentConversationMessageItem = {
  ...acknowledgementMessage,
  semantic: { purpose: 'conversation' },
}
void mismatchedAcknowledgement
// @ts-expect-error self-introduction messages must be authored by an Agent
const humanIntroduction: AgentConversationMessageItem = {
  ...selfIntroductionMessage,
  author: { participantId: 'human-1', role: 'human', displayName: { key: 'human.one', fallback: 'Human' } },
}
