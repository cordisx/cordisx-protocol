...
const activeRun: AgentConversationActiveRunDescriptor = {
...
const reaction: AgentConversationReaction = {
...
const message: AgentConversationItem = {
  ...
  source: 'agent-loop',
  author: participant,
  body: [{ kind: 'text', text: { key: 'message.one', fallback: 'Done' } }],
  reactions: [reaction],
...
const presence: AgentConversationMemberPresenceItem = {
  ...
  state: 'failed',
  retryable: true,
  diagnostic: { key: 'presence.failed', fallback: 'Agent failed to join' },
  retry: { id: 'chatroom:retry-member', arguments: { memberId: 'member-1', runId: 'run-1' } },
}
void message
void presence
source.dispose()
...
const humanAgentIdentity: AgentConversationParticipant = {
  participantId: 'human-1',
  role: 'human',
  displayName: { key: 'human.name', fallback: 'Human' },
  agentIdentity: { agentId: 'reviewer', revision: 'definition-4' },
}
void humanAgentIdentity
const leakedBinding: AgentConversationActiveRunDescriptor = {
  ...activeRun,
  // @ts-expect-error Host shell active runs do not expose the private AgentLoop binding
  taskDetails: { bindingId: 'binding-private', generation: 1 },
}
void leakedBinding
...
const invalidPresenceRetry: AgentConversationMemberPresenceItem = {
  ...presence,
  state: 'creating',
  retryable: true,
  retry: { id: 'chatroom:retry-member' },
}
