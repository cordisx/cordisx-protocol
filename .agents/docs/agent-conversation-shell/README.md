...
and error states, and avatar fallback. The plugin supplies only bounded data.

Only explicit multi-participant rooms may request `host-initials`; no avatar,
image, URL, HTML, CSS, component, callback, selector, rich media, Connector
conversation/run handle, or renderer projection exists. Composer unsent text
is Host-ephemeral. The Host generates bounded plain-text submit payload, and
v1 has no per-keystroke stream or draft persistence.
...
| Room, participants and initials opt-in | snapshot closed union; single-participant initials negative case |
