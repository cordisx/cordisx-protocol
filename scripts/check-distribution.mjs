...
try {
  ...
  if (JSON.stringify(actualFiles) !== JSON.stringify(expectedFiles)) {
    throw new Error(`unexpected publish contents: ${JSON.stringify(actualFiles)}`)
  }

  const packedArchive = packEntries(run(process.execPath, [npmCli, 'pack', '--json', '--pack-destination', temp]))
  if (JSON.stringify(packedArchive[0].files.map((file) => file.path).sort()) !== JSON.stringify(expectedFiles)) {
    throw new Error('actual pack contents drifted from dry-run')
  }
  if (!packedArchive[0].integrity || !packedArchive[0].shasum) {
    throw new Error('actual pack omitted integrity or shasum')
  }
...
declare const taskApprovals: AgentTaskApprovals, taskHandlers: AgentTaskApprovalHandlers, taskOwnership: AgentTaskOwnership
taskApprovals.register({ commandId: 'chatroom' }, taskHandlers)
taskApprovals.recover({ operationId: 'operation' })
taskOwnership.acquire({ operationId: 'operation' }).then(result => { if (result.status === 'acquired') result.handle.agent.session.id satisfies string })
declare const tasks: AgentTasks, taskRequest: AgentTaskCreateRequest
tasks.createAndSubmit(taskRequest)
tasks.query({ operationId: taskRequest.operationId }).then(result => { result satisfies AgentTaskQueryResult })
...
`,
  ...
  if (JSON.stringify(installed.exports) !== JSON.stringify(manifest.exports)) {
    throw new Error('consumer resolved different public exports')
  }
  ...
  if (fileDigest(installedRoot, frozenAgentLoopFiles) !== frozenAgentLoopDigest) {
    throw new Error('installed frozen AgentLoop v1/v2/v3 package bytes drifted')
  }
  ...
  console.log(
    JSON.stringify({
      npmVersion: run(process.execPath, [npmCli, '--version']).trim(),
      files: actualFiles,
      package: `${manifest.name}@${manifest.version}`,
      integrity: packedArchive[0].integrity,
      shasum: packedArchive[0].shasum,
      consumerImports: expectedExports,
    }),
  )
} finally {
