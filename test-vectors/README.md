...
Generic Manager collection suites live in `manager-collection/valid` and
`manager-collection/invalid`. They cover stable views, exact Host search
fields/normalization, query/snapshot fencing, same-owner row routes, unique
row/action identities, danger confirmation, text-input argument injection,
closed action results, and rejection of private renderer or business payloads.

Manager Content navigation v2 suites live in
`manager-content-navigation-v2/valid` and `manager-content-navigation-v2/invalid`.
They cover explicit localized tab-label precedence, v1-compatible route-title
derivation when absent, unchanged projection v1, and rejection of label
smuggling into v1 or an unversioned tab description.
