...
const surfaceIdentities = new Map([
  [schemaBase + 'surface-contribution.v9.schema.json', 9],
  [schemaBase + 'surface-contribution.v11.schema.json', 11],
])
...
const schemaErrors = (validate, value) =>
  validate(value) ? [] : (validate.errors ?? []).map(error => String(error.instancePath || '/') + ' ' + error.message)
...
function validateRegistration(registration) {
  ...
  if (!hasSchema) {
    ...
    const document = {
      $schema: schemaBase + 'surface-contribution.v8.schema.json',
      schemaVersion: 8,
      id: options?.id,
    ...
    errors.push(...schemaErrors(validator('surface-contribution.v8.schema.json'), document))
    return errors
  }
  if (surfaceIdentities.get(options.$schema) !== options.schemaVersion) {
    errors.push('versioned registration must use an accepted exact surface identity')
    return errors
  }
  const document = {
  ...
  errors.push(...schemaErrors(validator('surface-contribution.v' + options.schemaVersion + '.schema.json'), document))
  return errors
}
  ...
  return registrations.map(({ owner, options, item }) => {
    ...
    return {
      ...
      surfaceProvenance: versioned
        ? { kind: 'versioned', $schema: options.$schema, schemaVersion: options.schemaVersion }
      ...
      assignment: declaredGroup !== undefined ? 'declared' : versioned ? 'unassigned-fallback' : 'legacy-fallback',
...
function validateSuite(suite) {
  const errors = []
  errors.push(...schemaErrors(validator('manager-settings-navigation-groups.v2.schema.json'), suite.catalog))
  errors.push(...schemaErrors(validator('host-extension-point-catalog.v11.schema.json'), suite.extensionCatalog))
  ...
  errors.push(...schemaErrors(validator('manager-settings-navigation-projection.v3.schema.json'), suite.projection))
  if (errors.length > 0) return errors
  const descriptor = suite.extensionCatalog.points.find(point => point.id === 'manager.settings.navigation-items')
  if (!equal(descriptor?.navigationGroups, suite.catalog)) errors.push('extension catalog group catalog drift')
  if (!equal(suite.projection.catalog, suite.catalog)) errors.push('projection group catalog drift')
  if (!equal(suite.projection.contributions, expectedProjection(suite.registrations, suite.catalog))) {
    errors.push('runtime provenance projection drift')
  }
  return errors
}
...
const directory = path.join(root, 'test-vectors/manager-settings-navigation-groups-v2')
const valid = JSON.parse(await readFile(path.join(directory, 'valid/grouped.json'), 'utf8'))
assert.deepEqual(valid.catalog.groups.map(group => [group.id, group.order]), [
  ['resources', 100],
  ['development', 200],
  ['collaboration', 300],
...
console.log('Manager Settings navigation groups v2: all vectors passed')
