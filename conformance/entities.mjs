...
const validators = Object.fromEntries(
  schemaNames.filter(name =>
    name.startsWith('entity-') || name === 'plugin-package.v5.schema.json'
    || name === 'agent-acquire-request.v1.schema.json' || name === 'agent-definition.v1.schema.json'
    || name === 'session-event.v1.schema.json'
...
assert.equal(
  entityTreeDigest([...digestVector.files].reverse()),
  digestVector.expectedDigest,
  'digest order must be canonical',
)
assert.notEqual(
  entityTreeDigest(
    digestVector.files.map(file =>
      file.path === 'prompts/role.md' ? { ...file, content: `${file.content}changed` } : file
    ),
  ),
  digestVector.expectedDigest,
)
...
const promptFiles = [{
  ...
  text: digestVector.files.find(file => file.path === 'prompts/role.md').content,
}]
export function promptFileSetErrors(file, files) {
  const referenced = new Set(
    (file.promptSections ?? []).flatMap(section => section.source.kind === 'markdown' ? [section.source.path] : []),
  )
...
assert.ok(
  promptFileSetErrors(entityFile, [...promptFiles, { path: './prompts/extra.md', text: 'extra' }]).includes(
...
for (
  const invalidPath of [
    ...
    '../role.md',
    './prompts/../role.md',
    'https://example.test/role.md',
    './prompts/role.markdown',
  ]
) {
...
for (const forbidden of ['identity', 'revision', 'digest', 'expectedDigest']) {
  const invalid = {
    ...entityFile,
    [forbidden]: forbidden === 'identity'
      ? { agentId: 'reviewer', revision: digestVector.expectedDigest }
      : digestVector.expectedDigest,
  }
  assert.notDeepEqual(
    errors(validators['entity-file.v1.schema.json'], invalid),
    [],
    `${forbidden} must not enter editable source`,
  )
}
...
assert.deepEqual(
  entityPathErrors({
    entityRootRealPath: '/data/profiles/work/entities/reviewer',
    declaredPaths: ['./prompts/role.md'],
    realPaths: new Map([['./prompts/role.md', '/data/profiles/work/entities/reviewer/prompts/role.md']]),
  }),
  [],
)
assert.ok(
  entityPathErrors({
    entityRootRealPath: '/data/profiles/work/entities/reviewer',
    declaredPaths: ['./prompts/role.md'],
    realPaths: new Map([['./prompts/role.md', '/private/secret.md']]),
  }).includes('symlink-escape'),
)

export function templatePathErrors(packageRootRealPath, declaration, entityFileRealPath) {
  const issues = []
  if (!/^\.\/entities\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}\/entity\.json$/u.test(declaration.entityPath)) {
    issues.push('template-path-invalid')
  }
...
assert.deepEqual(
  templatePathErrors(
    ...
    '/packages/chatroom/entities/reviewer/entity.json',
  ),
  [],
)
assert.ok(
  templatePathErrors('/packages/chatroom', { entityPath: './entities/reviewer/entity.json' }, '/private/entity.json')
    .includes('symlink-escape'),
)
...
const record = {
  ...
  digest: digestVector.expectedDigest,
  definition,
  owner,
  access: 'owned',
  origin: 'local',
...
const persistedBinding = {
  ...
  owner,
  resolution: { identity: record.identity, digest: record.digest, definition: record.definition },
}
...
export function visibleEntities(currentBinding, records, shares = new Set()) {
  return records.filter(candidate => {
    ...
    const own = candidate.owner.installationId === currentBinding.installationId
      && candidate.owner.pluginId === currentBinding.pluginId
    const shareKey = [
      currentBinding.profileId,
      currentBinding.installationId,
      currentBinding.pluginId,
      candidate.owner.installationId,
...
const foreign = {
  ...record,
  identity: { agentId: 'foreign', revision: record.digest },
  definition: { ...definition, identity: { agentId: 'foreign', revision: record.digest } },
  owner: { profileId: 'work', installationId: 'install-other-1', pluginId: 'other' },
  access: 'shared-read',
}
const otherProfile = { ...foreign, owner: { ...foreign.owner, profileId: 'personal' } }
assert.deepEqual(visibleEntities(binding, [record, foreign, otherProfile]).map(value => value.identity.agentId), [
  'reviewer',
])
const share = [
  binding.profileId,
  ...
  binding.pluginId,
  foreign.owner.installationId,
  foreign.owner.pluginId,
  foreign.identity.agentId,
].join('\0')
assert.deepEqual(
  visibleEntities(binding, [record, foreign, otherProfile], new Set([share])).map(value => value.identity.agentId),
  ['reviewer', 'foreign'],
)
...
for (const field of ['profileId', 'installationId', 'pluginId', 'root', 'path', 'owner']) {
  assert.notDeepEqual(
    errors(validators['entity-registry-request.v1.schema.json'], { ...saveRequest, [field]: 'forbidden' }),
    [],
    `${field} must not be caller-selected`,
  )
}
const applied = {
...
for (
  const entityPath of [
    '/entities/reviewer/entity.json',
    '../reviewer/entity.json',
    './entities/reviewer/../entity.json',
  ]
) {
  assert.notDeepEqual(
    errors(validators['entity-template-declaration.v1.schema.json'], { ...declaration, entityPath }),
    [],
  )
}
const packageManifest = {
  ...
  runtimeManifest: {
    ...
    schema: 'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/plugin-manifest.v5.schema.json',
...
export function templateDeclarationErrors(manifest) {
  ...
  if ((manifest.entityTemplates?.length ?? 0) > 0 && !manifest.compatibility.protocolSchemas.includes(schemaId)) {
    issues.push('entity schema missing from compatibility')
  }
...
assert.ok(
  templateDeclarationErrors({ ...packageManifest, entityTemplates: [{ ...declaration, agentId: 'writer' }] }).length
    > 0,
)
assert.ok(
  templateDeclarationErrors({
    ...packageManifest,
    entityTemplates: [declaration, { ...declaration, digest: `sha256:${'9'.repeat(64)}` }],
  }).includes('duplicate template agent id'),
)
...
assert.deepEqual(
  materializeTemplate(localStore, {
    ...template,
    payload: { entity: { ...entityFile, name: 'Package update' }, promptFiles },
  }),
  { status: 'preserved', code: 'entity-present' },
)
assert.equal(
  localStore.get('reviewer').entity.name,
  'User edited reviewer',
  'package update must not overwrite local edits',
)
...
const page = {
  ...
  subscription,
  phase: 'replay',
  changes: [{ kind: 'entity-updated', sequence: 5, entity: record }, {
    kind: 'entity-removed',
    sequence: 6,
...
) assert.deepEqual(errors(validators['entity-registry-subscription-close.v1.schema.json'], close(code)), [])
...
assert.deepEqual(
  subscriptionLifecycleErrors([{ kind: 'page', value: page }, { kind: 'closed', value: close('unsubscribed') }]),
  [],
)
assert.ok(
  subscriptionLifecycleErrors([{ kind: 'closed', value: close('connection-replaced') }, { kind: 'page', value: page }])
    .length > 0,
)
...
assert.notDeepEqual(
  errors(validators['entity-agent-acquire-request.v1.schema.json'], {
    ...acquireRequest,
    definition: { ...record.identity, revision: `sha256:${'2'.repeat(64)}` },
    setup: { definition: record.identity, definitions: [definition] },
  }),
  [],
)
const acquireResult = {
...
assert.deepEqual(
  errors(validators['entity-agent-acquire-request.v1.schema.json'], resumeRequest),
  [],
  'entity-backed resume does not require a mutable local identity',
)
const resumeResult = {
  ...acquireResult,
  operation: 'resume',
...
const invalidResumeMissingLocal = {
  $schema: acquireResult.$schema,
  contract: acquireResult.contract,
  schemaVersion: 1,
  operation: 'resume',
  status: 'unavailable',
...
assert.notDeepEqual(
  errors(validators['entity-agent-acquire-result.v1.schema.json'], invalidResumeMissingLocal),
  [],
  'resume cannot report entity-not-found merely because local source disappeared',
)
...
export function resolveCreate(request, records) {
  ...
  if (candidate.identity.revision !== request.definition.revision || candidate.digest !== request.definition.revision) {
    return { status: 'unavailable', code: 'entity-revision-stale' }
  }
  if (candidate.definition.identity.revision !== candidate.digest) {
    return { status: 'unavailable', code: 'entity-invalid' }
  }
  return {
    status: 'accepted',
    definitionResolution: { identity: candidate.identity, digest: candidate.digest, definition: candidate.definition },
  }
}
assert.equal(resolveCreate(acquireRequest, [record]).status, 'accepted')
assert.deepEqual(
  resolveCreate({ ...acquireRequest, definition: { ...record.identity, revision: `sha256:${'2'.repeat(64)}` } }, [
    record,
  ]),
  { status: 'unavailable', code: 'entity-revision-stale' },
)
...
export function resolveResume(request, persistedBySession) {
  ...
  if (
    request.definition
    && (request.definition.agentId !== persisted.resolution.identity.agentId
      || request.definition.revision !== persisted.resolution.identity.revision)
  ) return { status: 'unavailable', code: 'entity-revision-stale' }
...
const recordB = {
  ...record,
  identity: { agentId: record.identity.agentId, revision: revisionB },
  digest: revisionB,
  definition: {
    ...record.definition,
    identity: { agentId: record.identity.agentId, revision: revisionB },
...
assert.equal(
  resolveCreate({ ...acquireRequest, definition: record.identity }, [recordB]).code,
  'entity-revision-stale',
  'new create cannot use old A after local edit to B',
)
assert.equal(
  resolveCreate({ ...acquireRequest, definition: recordB.identity }, [recordB]).status,
  'accepted',
  'new create uses current B',
)
assert.equal(resolveCreate(acquireRequest, []).code, 'entity-not-found', 'new create cannot use removed local A')
assert.deepEqual(resolveResume({ type: 'resume', sessionId: definitionBoundEvent.sessionId }, persistedBySession), {
  status: 'accepted',
  definitionSource: 'session-persisted',
  definitionResolution: persistedBinding.resolution,
}, 'resume succeeds as A after local edit to B')
assert.equal(
  resolveResume(
    { type: 'resume', sessionId: definitionBoundEvent.sessionId, definition: record.identity },
    persistedBySession,
  ).status,
  'accepted',
)
assert.equal(
  resolveResume(
    { type: 'resume', sessionId: definitionBoundEvent.sessionId, definition: recordB.identity },
    persistedBySession,
  ).code,
  'entity-revision-stale',
  'resume never silently switches to B',
)
assert.equal(
  resolveResume({ type: 'resume', sessionId: definitionBoundEvent.sessionId }, persistedBySession).status,
  'accepted',
  'resume succeeds as A after local entity removal',
)
...
assert.deepEqual(
  errors(validators['agent-acquire-request.v1.schema.json'], inlineSetupRequest),
  [],
  'frozen inline AgentSetup remains valid',
)

console.log(
  'Entities v1 conformance: local editable source, deterministic digest, scoped registry, template no-overwrite, exact Agent resolution, and closure fences passed',
)
