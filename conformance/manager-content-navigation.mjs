...
for (const [name, validator] of Object.entries(validators)) if (validator === undefined) throw new Error(`${name} schema was not registered`)

const LOCAL_ID = /^[a-z0-9][a-z0-9._-]{0,95}$/
const ROUTE_PARAM = /:([a-z][a-zA-Z0-9]*)/g
const equal = (left, right) => JSON.stringify(left) === JSON.stringify(right)
const qualified = (owner, id) => `${owner}:${id}`
const schemaErrors = (validator, value) => validator(value) ? [] : (validator.errors ?? []).map(error => `${error.instancePath || '/'} ${error.message}`)
...
function routeRefMatches(reference, qualifiedId, owner, localId, params) {
  return reference?.id === qualifiedId && equal(reference.params ?? {}, params ?? {}) && qualifiedId === qualified(owner, localId)
}
...
export function validateManagerContentNavigationSuite(suite) {
  ...
  for (const [index, owner] of (Array.isArray(suite.owners) ? suite.owners : []).entries()) {
    if (typeof owner?.id !== 'string' || !LOCAL_ID.test(owner.id) || owner.id === 'host' || owner.id.startsWith('cordisx.')) errors.push(`owners[${index}] uses a reserved or invalid owner id`)
  ...
  documentErrors(errors, 'declarations', suite.declarations, validators['manager-content-navigation.v1.schema.json'], owners, declarations)
  ...
  for (const [key, declaration] of declarations) {
    ...
    if (route.outlet !== 'manager.content' || !route.path.startsWith('/manager/')) errors.push(`declaration ${key} must target a Manager content route`)
    ...
    else if ((page.chrome ?? 'standard') !== 'standard') errors.push(`declaration ${key} requires a standard page shell`)
    ...
    if (declaration.header?.title?.kind === 'record') {
      ...
      if (!parameters.has(declaration.header.title.recordIdParam)) errors.push(`declaration ${key} recordIdParam is not a named route parameter`)
      if (!Object.hasOwn(declaration.route.params ?? {}, declaration.header.title.recordIdParam)) errors.push(`declaration ${key} record title lacks its current route parameter`)
    }
  ...
  errors.push(...schemaErrors(validators['manager-content-projection.v1.schema.json'], suite.projection).map(error => `projection schema: ${error}`))
  ...
  if (!routeRefMatches(projection?.route, qualified(owner, active.route.id), owner, active.route.id, active.route.params)) errors.push('projection route does not match the active declaration')
  const headerTitle = projection?.header?.title
  if (active.header?.title?.kind === 'route' && headerTitle?.kind !== 'route') errors.push('route title declaration requires a route title projection')
  if (active.header?.title?.kind === 'record') {
    ...
    else if (headerTitle.recordId !== String(active.route.params?.[active.header.title.recordIdParam])) errors.push('record title projection must use the active opaque route parameter')
  }
  const parent = active.parentRoute
  if (parent === undefined && projection?.back?.available === true) errors.push('root Manager content declaration must not project a plugin back route')
  if (parent !== undefined) {
    const expectedParent = qualified(owner, parent.id)
    if (projection?.back?.available !== true || !routeRefMatches(projection.back.route, expectedParent, owner, parent.id, parent.params)) errors.push('Host back projection must target the declared parent route')
    const lastBreadcrumb = projection?.breadcrumbs?.at(-1)
    if (!routeRefMatches(lastBreadcrumb?.route, expectedParent, owner, parent.id, parent.params)) errors.push('Host breadcrumb projection must retain the declared parent route')
  }
  const history = projection?.history
  if (history !== undefined && (history.canGoBack !== (history.index > 0) || history.canGoForward !== (history.index < history.length - 1))) errors.push('history flags must be derived from the Host history cursor')
  const outputTabs = projection?.tabs ?? []
  if ((active.tabs?.length ?? 0) !== outputTabs.length) errors.push('projection tabs must exactly map the declaration tabs')
  ...
  for (const [index, tab] of (active.tabs ?? []).entries()) {
    ...
    if (output?.id !== tab.id || !routeRefMatches(output?.route, qualified(owner, tab.route.id), owner, tab.route.id, tab.route.params)) errors.push(`projection tab ${tab.id} does not map its declared route reference`)
  }
  return errors
}

async function jsonFiles(directory) {
  return (await readdir(directory, { withFileTypes: true })).filter(entry => entry.isFile() && entry.name.endsWith('.json')).map(entry => path.join(directory, entry.name)).sort()
}
...
for (const file of await jsonFiles(path.join(directory, 'valid'))) {
  ...
  if (errors.length > 0) { console.error(`${path.relative(root, file)} should be valid`, errors); failures += 1 }
}
for (const file of await jsonFiles(path.join(directory, 'invalid'))) {
  const errors = validateManagerContentNavigationSuite(materializeInvalidVector(JSON.parse(await readFile(file, 'utf8')), completeSuite))
  if (errors.length === 0) { console.error(`${path.relative(root, file)} should be invalid`); failures += 1 }
}
