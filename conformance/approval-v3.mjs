...
assert.deepEqual(structuredClone({ registration, question, accepted, unavailable, closed }), {
  registration,
  question,
  accepted,
  unavailable,
...
export function routingErrors({ question: routedQuestion, result, expectedRegistration, expectedOwner, liveBindings }) {
  ...
  if (!same(routedQuestion.registration.requester, routedQuestion.requester)) {
    issues.push('registered requester mismatch')
  }
  if (routedQuestion.requester.agentId !== routedQuestion.requester.sessionId) {
    issues.push('requester AgentId must equal SessionId')
  }
  if (result.status === 'accepted') {
    ...
    if (!same(liveBindings.get(bindingKey(result.requester)), expectedOwner)) {
      issues.push('requester live owner mismatch')
    }
    if (!same(liveBindings.get(bindingKey(result.authority)), expectedOwner)) {
      issues.push('authority live owner mismatch')
    }
  }
  return issues
}
...
assert.notDeepEqual(
  routingErrors({ ...validRouting, expectedOwner: { ...owner, pluginId: 'foreign' } }),
  [],
  'foreign owner fails closed',
)
assert.notDeepEqual(
  routingErrors({ ...validRouting, expectedOwner: { ...owner, generation: 10 } }),
  [],
  'replacement owner generation fails closed',
)
assert.notDeepEqual(
  routingErrors({ ...validRouting, expectedRegistration: { ...registration, registrationId: 'replacement' } }),
  [],
...
assert.notDeepEqual(
  routingErrors({
    ...validRouting,
    question: { ...question, requester: { ...reviewer, sessionId: 'foreign-session' } },
  }),
  [],
  'wrong requester Session fails closed',
)
assert.notDeepEqual(
  routingErrors({ ...validRouting, question: { ...question, requester: { ...reviewer, agentGeneration: 5 } } }),
  [],
  'stale requester generation fails closed',
)
assert.notDeepEqual(
  routingErrors({ ...validRouting, result: { ...accepted, authority: { ...lead, agentGeneration: 8 } } }),
  [],
  'stale authority generation fails closed',
)
assert.notDeepEqual(
  routingErrors({ ...validRouting, result: { ...accepted, authority: { ...lead, definition: reviewer.definition } } }),
  [],
  'wrong authority definition fails closed',
)
...
assert.notDeepEqual(
  routingErrors({ ...validRouting, liveBindings: foreignAuthorityBindings }),
  [],
  'foreign authority owner fails closed',
)
  ...
  const invalid of [
    {
      ...accepted,
      routingId: question.routingId,
      authority: { ...lead, definition: { agentId: '*', revision: lead.definition.revision } },
    },
...
class ResolverLedger {
  ...
  async resolve(routedQuestion) {
    ...
    try {
      ...
      return validator('approval-request-routing-result.v1.schema.json')(result)
          && routingErrors({
              question: routedQuestion,
              result,
              expectedRegistration: record.registration,
              expectedOwner: owner,
              liveBindings,
            }).length === 0
...
assert.equal(
  await ledger.resolve({ ...question, requester: neverRegistered }),
  'legacy-v1',
  'never-adopted requester preserves frozen v1 behavior',
)
...
const replacementAccepted = {
  ...
  routingId: replacementQuestion.routingId,
}
replacementLedger.register(reviewer, () => replacementAccepted, replacementRegistration)
oldDispose()
assert.equal(
  await replacementLedger.resolve(replacementQuestion),
  'approval-v2',
  'disposing a replaced handle cannot close its successor',
)
assert.equal(
  await replacementLedger.resolve(question),
  'fail-closed',
  'a stale registration cannot invoke its replacement',
)

const persistence = []
ledger.register(reviewer, () => {
  persistence.push('resolved')
  return accepted
})
assert.equal(await ledger.resolve(question), 'approval-v2')
...
assert.deepEqual(
  persistence,
  ['resolved', 'approval/authority-bound', 'approval/asked', 'approval/decided'],
  'routing must finish before the one durable approval chain',
)
...
assert.equal(
  digest.digest('hex'),
  'f914b1d5d35798114d3e3e76c1cba20ba1bb8ffc3285d8b04c06ab6720ecf89a',
  'Approval v1-v2 public bytes drifted',
)
