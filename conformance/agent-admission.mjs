...
const names = [
  ...
  'agent-conversation-shell-common.v7.schema.json',
  'agent-command-origin.v1.schema.json',
  'agent-bootstrap-command-origin.v1.schema.json',
  'agent-admission-receipt.v1.schema.json',
  'agent-admission-capture-close.v1.schema.json',
  'agent-admission-capture-result.v1.schema.json',
  'agent-conversation-shell-command-context.v8.schema.json',
  'agent-conversation-shell-command-context.v9.schema.json',
  'agent-admission-target-origin.v3.schema.json',
  'agent-admission-target-reservation.v3.schema.json',
  'agent-admission-bootstrap-target-origin.v4.schema.json',
  'agent-admission-bootstrap-reservation.v4.schema.json',
  'agent-admission-bootstrap-room-target-origin.v5.schema.json',
  'agent-admission-bootstrap-room-reservation.v5.schema.json',
  'agent-admission-bootstrap-room-target-receipt.v5.schema.json',
...
const origin = {
  ...
  generation: 'shell-1',
  executionId: 'exec-1',
  commandId: 'chatroom.message.submit',
  scope: 'composer-submit',
  room: { roomId: 'room-4', participantId: 'participant-lead', memberId: 'member-lead', runId: 'run-4' },
}
const receipt = {
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-admission-receipt.v1.schema.json',
  contract: 'cordisx.agent-admission-receipt/v1',
  schemaVersion: 1,
  receiptId: 'receipt-1',
...
const context = {
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-conversation-shell-command-context.v8.schema.json',
  contract: 'cordisx.agent-conversation-shell-command-context/v8',
  schemaVersion: 8,
...
assert.equal(
  validate('agent-conversation-shell-command-context.v8.schema.json')({ ...context, origin: undefined }),
  false,
)
const bootstrap = {
  $schema:
  ...
  schemaVersion: 1,
  originId: 'bootstrap-1',
  binding: { bindingId: 'binding-bootstrap', ownerGeneration: 'owner-bootstrap' },
  generation: 'shell-bootstrap',
  executionId: 'execution-bootstrap',
  commandId: 'chatroom.message.submit',
  scope: 'composer-submit',
}
...
const bootstrapContext = {
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-conversation-shell-command-context.v9.schema.json',
  contract: 'cordisx.agent-conversation-shell-command-context/v9',
  schemaVersion: 9,
  binding: bootstrap.binding,
  generation: bootstrap.generation,
  scope: 'composer-submit',
  command: { id: bootstrap.commandId },
  submitPayload: 'first room',
  origin: bootstrap,
}
assert.equal(
  validate('agent-conversation-shell-command-context.v9.schema.json')(bootstrapContext),
  true,
  'fresh Room bootstrap has no active Run or Session',
)
const targetOrigins = ['leader/run-1', 'reviewer/run-2', 'integrator/run-3'].map((target, index) => ({
  $schema:
  ...
  schemaVersion: 3,
  token: `host-issued-${index}-${target}`,
}))
for (const targetOrigin of targetOrigins) {
  assert.equal(validate('agent-admission-target-origin.v3.schema.json')(targetOrigin), true)
}
assert.equal(
  new Set(targetOrigins.map(value => value.token)).size,
  3,
  'each delivery receives a distinct Host-issued capability',
)
const targetReservation = (origin) => ({
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-admission-target-reservation.v3.schema.json',
  contract: 'cordisx.agent-admission-target-reservation/v3',
  schemaVersion: 3,
  reservationId: `reservation-${origin.token}`,
  origin,
...
const bootstrapOrigins = [1, 2, 3].map(index => ({
  $schema:
  ...
  schemaVersion: 4,
  token: `bootstrap-target-${index}`,
}))
for (const targetOrigin of bootstrapOrigins) {
  assert.equal(validate('agent-admission-bootstrap-target-origin.v4.schema.json')(targetOrigin), true)
}
assert.equal(
  new Set(bootstrapOrigins.map(value => value.token)).size,
  3,
  'one bootstrap command issues distinct origins for N=3 new targets',
)
...
const consumeBootstrap = (target, origin) => {
  ...
  bootstrapUsed.add(origin.token)
  return 'reserved'
}
for (const targetOrigin of bootstrapOrigins) {
  assert.equal(consumeBootstrap(bootstrapOrigins.indexOf(targetOrigin), targetOrigin), 'reserved')
}
...
assert.equal(
  validate('agent-conversation-shell-command-context.v9.schema.json')({ ...bootstrapContext, origin: undefined }),
  false,
  'predecessor no-origin path remains unavailable',
)
const roomTargets = [
  ...
  {
    roomId: 'room-fresh',
    participantId: 'participant-integrator',
    memberId: 'member-integrator',
    runId: 'run-integrator',
  },
]
const roomOrigins = roomTargets.map((target, index) => ({
  $schema:
  ...
  contract: 'cordisx.agent-admission-bootstrap-room-target-origin/v5',
  schemaVersion: 5,
  token: `room-fresh-${index}`,
}))
for (const value of roomOrigins) {
  assert.equal(validate('agent-admission-bootstrap-room-target-origin.v5.schema.json')(value), true)
}
const roomReceipt = {
  $schema:
  ...
  contract: 'cordisx.agent-admission-bootstrap-room-target-receipt/v5',
  schemaVersion: 5,
  receiptId: 'room-receipt-1',
  target: roomTargets[0],
}
...
const issueRoom = (target, origin) => {
  ...
  issuedRoomTargets.add(key)
  return origin.token.startsWith('room-fresh-') ? 'issued' : 'target-denied'
}
...
const routeTargets = roomTargets.map(target => ({
  ...target,
  route: { routeId: 'room', param: 'roomId', roomId: target.roomId },
}))
const continuations = routeTargets.map((target, index) => ({
  $schema:
  ...
  contract: 'cordisx.agent-admission-bootstrap-route-continuation/v6',
  schemaVersion: 6,
  token: `route-continuation-${index}`,
}))
for (const continuation of continuations) {
  assert.equal(validate('agent-admission-bootstrap-route-continuation.v6.schema.json')(continuation), true)
}
const routeReservation = {
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-admission-bootstrap-route-reservation.v6.schema.json',
  contract: 'cordisx.agent-admission-bootstrap-route-reservation/v6',
  schemaVersion: 6,
  reservationId: 'route-reservation-1',
  continuation: continuations[0],
  message: { text: 'dispatch text' },
}
assert.equal(validate('agent-admission-bootstrap-route-reservation.v6.schema.json')(routeReservation), true)
const routeClaimReceipt = {
  $schema:
    'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/agent-admission-bootstrap-route-claim-receipt.v6.schema.json',
  contract: 'cordisx.agent-admission-bootstrap-route-claim-receipt/v6',
  schemaVersion: 6,
  receiptId: 'route-claim-1',
  owner: { pluginId: 'chatroom', generation: 9 },
  origin: bootstrap,
  target: routeTargets[0],
  binding: {
    binding: { bindingId: 'binding-room-fresh', ownerGeneration: 'owner-bootstrap' },
    generation: 'shell-room-fresh',
    route: routeTargets[0].route,
  },
  source: { sessionId: 'session-lead', messageId: 'cx-message' },
}
...
assert.equal(
  validate('agent-admission-bootstrap-route-claim-receipt.v6.schema.json')({
    ...routeClaimReceipt,
    target: { ...routeClaimReceipt.target, route: { ...routeClaimReceipt.target.route, param: 'sessionId' } },
  }),
  false,
  'unknown Room route parameter fails schema validation',
)
assert.equal(
  validate('agent-admission-bootstrap-route-claim-receipt.v6.schema.json')({
    ...routeClaimReceipt,
    source: { ...routeClaimReceipt.source, extra: 'forbidden' },
  }),
  false,
  'unbounded source fields fail schema validation',
)
const openRouteCommand = count => {
  const targets = routeTargets.slice(0, count)
  const commandContinuations = targets.map((_, index) => ({
    ...continuations[index],
    token: `command-${count}-${index}`,
  }))
  ...
  const declare = (target, continuation) => {
    ...
    declared.set(key, { target, continuation })
    return 'declared'
  }
  const claim = (continuation, route, source, fence) => {
    ...
    if (fence) {
      rejected.add(continuation.token)
      return fence
    }
    const value = [...declared.values()].find(entry => entry.continuation === continuation)
    if (!value) return 'continuation-denied'
    if (!submitted.has(continuation.token)) {
      rejected.add(continuation.token)
      return 'not-submitted'
    }
    if (
      !value || route.routeId !== value.target.route.routeId || route.param !== value.target.route.param
      || route.roomId !== value.target.roomId
    ) {
      rejected.add(continuation.token)
      return 'route-mismatch'
    }
    if (
      source.sessionId !== `session-${value.target.memberId.slice('member-'.length)}`
      || source.messageId !== 'cx-message'
    ) {
      rejected.add(continuation.token)
      return 'source-mismatch'
    }
    consumed.add(continuation.token)
    return 'claimed'
  }
...
for (const count of [1, 2, 3]) {
  ...
  for (const [index, target] of command.targets.entries()) {
    assert.equal(
      command.declare(target, command.continuations[index]),
      'declared',
      `v6 declares exact same-room target ${index + 1} of N=${count}`,
    )
  }
  for (const [index, target] of command.targets.entries()) {
    assert.equal(
      command.reserve(command.continuations[index], target),
      'reserved',
      `v6 reserves exact target ${index + 1} of N=${count}`,
    )
  }
  ...
  for (const [index, target] of command.targets.entries()) {
    ...
    assert.equal(
      command.claim(command.continuations[index], target.route, {
        sessionId: `session-${member}`,
        messageId: 'cx-message',
      }),
      'claimed',
      `v6 atomically transfers target ${index + 1} of N=${count}`,
    )
  }
}
...
assert.equal(
  beforeSubmit.claim(beforeSubmit.continuations[0], beforeSubmit.targets[0].route, {
    sessionId: 'session-lead',
    messageId: 'cx-message',
  }),
  'not-submitted',
  'claim before accepted submit fails closed',
)
assert.equal(
  beforeSubmit.claim(beforeSubmit.continuations[0], beforeSubmit.targets[0].route, {
    sessionId: 'session-lead',
  ...
  'reused',
  'a premature claim consumes the continuation',
)
...
assert.equal(
  crossRoom.claim(crossRoom.continuations[0], { ...crossRoom.targets[0].route, roomId: 'foreign-room' }, {
    sessionId: 'session-lead',
    messageId: 'cx-message',
  }),
  'route-mismatch',
  'cross-room rebinding is rejected and consumed',
)
...
assert.equal(
  crossTarget.reserve(crossTarget.continuations[0], crossTarget.targets[1]),
  'target-mismatch',
  'a continuation cannot reserve a different Room target',
)
assert.equal(
  crossTarget.reserve({ ...crossTarget.continuations[0], token: 'forged-continuation' }, crossTarget.targets[0]),
  'continuation-denied',
  'a forged continuation has no reservation authority',
)
crossTarget.submitted.add(crossTarget.continuations[0].token)
assert.equal(
  crossTarget.claim(crossTarget.continuations[0], crossTarget.targets[0].route, {
    sessionId: 'session-wrong',
    messageId: 'cx-message',
  }),
  'source-mismatch',
  'a claimed source must equal the accepted Session and message',
)
...
assert.equal(
  replaced.claim(replaced.continuations[0], replaced.targets[0].route, {
    sessionId: 'session-lead',
    messageId: 'cx-message',
  }, 'binding-replaced'),
  'binding-replaced',
  'a non-declared binding replacement cannot transfer the capture',
)
assert.equal(
  replaced.claim(replaced.continuations[0], replaced.targets[0].route, {
    sessionId: 'session-lead',
  ...
  'reused',
  'a fenced continuation cannot be reclaimed',
)
...
assert.equal(
  duplicate.declare({
    ...duplicate.targets[0],
    roomId: 'foreign-room',
    route: { ...duplicate.targets[0].route, roomId: 'foreign-room' },
  }, { ...duplicate.continuations[0], token: 'foreign-room' }),
  'cross-room',
)
const inconsistentRoute = openRouteCommand(1)
assert.equal(
  inconsistentRoute.declare({
    ...inconsistentRoute.targets[0],
    route: { ...inconsistentRoute.targets[0].route, roomId: 'wrong-room' },
  }, inconsistentRoute.continuations[0]),
  'route-denied',
