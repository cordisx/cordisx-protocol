...
assert.equal(
  ...
  'd47afe0df17efeea720827ada95c4cf1c772671db2e5085a4e28e469f0dfae46',
  'v1/v2 contract bytes must remain frozen',
)
...
assert.deepEqual(
  v3AsV2,
  schemas.get('manager-content-navigation.v2.schema.json'),
  'v3 must be an additive v2 declaration successor',
)
...
function validateSuite(suite) {
  ...
  for (const [index, record] of (suite.declarations ?? []).entries()) {
    ...
    if (
      (record.document.subject || record.document.recordSummary) && record.document.header?.title?.kind !== 'record'
    ) {
    ...
    if (record.document.subject?.kind === 'agent-definition') {
      ...
      if (subjects.has(subjectKey)) {
        errors.push(`duplicate agent-definition subject claim ${identity?.agentId}@${identity?.revision}`)
      }
  ...
  if (!equal(suite.projection?.recordSummary, active.recordSummary)) {
    errors.push('projection record summary must exactly match the active declaration')
  }
...
assert.deepEqual(
  changedTab.recordSummary,
  valid.projection.recordSummary,
  'tab selection must preserve the fixed record summary',
)
