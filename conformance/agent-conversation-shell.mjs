...
const schemas = new Map(
  await Promise.all(
    names.map(async name => [name, JSON.parse(await readFile(path.join(root, 'schemas', name), 'utf8'))]),
  ),
)
...
const validateDetailsUrl = ajv.getSchema(
  `${schemas.get('agent-loop-task-details-common.v2.schema.json').$id}#/$defs/detailsUrl`,
)
...
const action = id => ({
  ...
  label: text(id),
  icon: 'host:more',
  command: command(`chatroom:${id}`),
  disabled: { value: false },
})
const participant = {
  participantId: 'participant-1',
  role: 'agent',
  displayName: text('Ada'),
  avatar: { kind: 'asset', ref: 'asset:participant-1', revision: 'revision:avatar-1' },
  agentIdentity: { agentId: 'reviewer', revision: 'definition-4' },
}
...
const otherAgent = {
  ...
  role: 'agent',
  displayName: text('Other Agent'),
  agentIdentity: { agentId: 'writer', revision: 'definition-2' },
}
const activeRunA = {
  participantId: participant.participantId,
  memberId: 'member-1',
  runId: 'run-1',
  lifecycle: { phase: 'running', updatedAt: '2026-08-31T01:00:00.000Z' },
...
const activeRunB = {
  ...
  detailsUrl: { url: 'https://example.test/tasks/task-2', target: 'external' },
}
const pendingEmoji = {
  reactionId: 'reaction-emoji',
  actorParticipantId: humanParticipant.participantId,
  value: { kind: 'emoji', emoji: '👍🏽' },
  state: 'pending',
}
const pendingSemantic = {
  reactionId: 'reaction-semantic',
...
const message = {
  ...
  source: 'agent-loop',
  author: participant,
  body: [{ kind: 'text', text: text('Hello') }],
  reactions: [pendingEmoji, pendingSemantic],
...
const binding = {
  $schema: schemas.get('agent-conversation-shell-binding.v2.schema.json').$id,
  contract: 'cordisx.agent-conversation-shell-binding/v2',
  schemaVersion: 2,
  bindingId: 'binding-1',
  shell: 'agent-desktop',
  ownerGeneration: 'generation-1',
  routeSelection: { scope: 'room-or-new', selectedRoomParam: 'roomId' },
}
const snapshot = {
  $schema: snapshotSchemaId,
  contract: 'cordisx.agent-conversation-shell-snapshot/v2',
  schemaVersion: 2,
  ...
  selection: {
    ...
    participants: [participant, identitylessAgent, humanParticipant, otherAgent],
    activeRuns: [activeRunA, activeRunB],
  },
  items: [message, acknowledgement, presenceSuccess, presenceRetry],
  composer: {
    availability: 'available',
    placeholder: text('Message'),
    disabled: { value: false },
    submit: command('chatroom:submit'),
  },
  headerActions: [action('new-room'), action('manage-agents'), action('more')],
}
const subscription = {
  $schema: schemas.get('agent-conversation-shell-subscription.v2.schema.json').$id,
  contract: 'cordisx.agent-conversation-shell-subscription/v2',
  schemaVersion: 2,
...
const result = {
  $schema: schemas.get('agent-conversation-shell-result.v2.schema.json').$id,
  contract: 'cordisx.agent-conversation-shell-result/v2',
  schemaVersion: 2,
  requestId: 'request-1',
  type: 'subscribe',
  status: 'accepted',
...
const context = {
  $schema: schemas.get('agent-conversation-shell-command-context.v2.schema.json').$id,
  contract: 'cordisx.agent-conversation-shell-command-context/v2',
  schemaVersion: 2,
  binding: snapshot.binding,
  generation: 'generation-1',
  scope: 'composer-submit',
  command: command('chatroom:submit'),
  submitPayload: 'hello',
}
...
function reactionValueErrors(value) {
  ...
  if (value?.kind === 'semantic') {
    if (typeof value.token !== 'string' || !/^[a-z][a-z0-9.-]{0,31}$/.test(value.token)) {
      errors.push('reaction semantic token is not canonical')
    }
    return errors
  }
  ...
  for (const scalar of [...remainder]) {
    if (!allowedScalar.test(scalar)) errors.push('reaction emoji contains a non-emoji scalar')
  }
  if (!/\p{Extended_Pictographic}|\p{Regional_Indicator}/u.test(remainder) && keycaps.length === 0) {
    errors.push('reaction emoji has no complete emoji base')
  }
  return errors
}
...
function selectionAndItemErrors(current) {
  ...
  for (const run of current.selection.activeRuns ?? []) {
    ...
    else if (member.role !== 'agent' || member.agentIdentity === undefined) {
      errors.push('active run participant lacks exact Agent Definition identity')
    }
    const priorParticipantId = memberParticipants.get(run.memberId)
    if (priorParticipantId !== undefined && priorParticipantId !== run.participantId) {
      errors.push('memberId changed participantId')
    }
  ...
  for (let index = 0; index < current.items.length; index += 1) {
    ...
    if (item.kind === 'message') {
      ...
      else if (!isDeepStrictEqual(item.author, author)) {
        errors.push('message author overrides member-owned participant identity')
      }
      ...
      if (member === undefined) errors.push('presence participant is not current')
        ...
        errors.push('presence participant lacks exact Agent Definition identity')
      }
      const priorParticipantId = memberParticipants.get(item.memberId)
      if (priorParticipantId !== undefined && priorParticipantId !== item.participantId) {
        errors.push('memberId changed participantId')
      }
      ...
      if (['joined', 'ready'].includes(item.state) && !activeRunTriples.has(key)) {
        errors.push('joined or ready presence lacks an exact active run projection')
      }
    }
  }
  return errors
...
function snapshotErrors(value) {
  const errors = []
  if (!v['agent-conversation-shell-snapshot.v2.schema.json'](value)) {
    errors.push(ajv.errorsText(v['agent-conversation-shell-snapshot.v2.schema.json'].errors))
...
function reactionTransitionErrors(previous, next, participants) {
  ...
  for (let index = 0; index < next.length; index += 1) {
    ...
    if (previous[index]?.reactionId !== reaction.reactionId) {
      errors.push('reaction moved from its stable array position')
    }
    if (old.actorParticipantId !== reaction.actorParticipantId || !isDeepStrictEqual(old.value, reaction.value)) {
      errors.push('reaction actor or value drift')
    }
    if (old.state !== 'pending' && reaction.state !== old.state) {
      errors.push('terminal reaction regressed or changed outcome')
    }
    if (old.state === 'pending' && !['pending', 'completed', 'failed'].includes(reaction.state)) {
      errors.push('reaction transition is invalid')
    }
  }
  for (const reaction of previous) {
    if (!nextById.has(reaction.reactionId)) errors.push('reaction disappeared from message')
  }
  return errors
}
...
function itemTransitionErrors(previous, next, participants) {
  ...
  if (previous.kind === 'message') {
    ...
    ) errors.push('presence relation drift')
    if (!presenceTransitions[previous.state]?.has(next.state)) errors.push('presence transition is invalid')
    if (previous.state === 'failed' && !previous.retryable && next.state !== 'failed') {
      errors.push('non-retryable failure is terminal')
    }
    if (previous.state === next.state && !isDeepStrictEqual(previous, next)) {
      errors.push('same-state presence update is not idempotent')
    }
  }
  return errors
}
...
function applyItemUpdate(current, update) {
  ...
  const participants = next.selection.kind === 'room'
    ? new Map(next.selection.participants.map(value => [value.participantId, value]))
    : new Map()
  if (update.kind === 'item-appended') {
    if (next.items.some(item => item.itemId === update.item.itemId)) {
      return { snapshot: next, errors: ['item-appended reused itemId'] }
    }
    if (
      update.item.kind === 'message'
      && next.items.some(item => item.kind === 'message' && item.messageId === update.item.messageId)
    ) return { snapshot: next, errors: ['item-appended reused messageId'] }
    if (update.item.sequence !== next.items.length) {
      return { snapshot: next, errors: ['item-appended does not append at timeline tail'] }
    }
...
const v1Message = {
  ...
  sequence: 0,
  author: v1Participant,
  body: [{ kind: 'text', text: text('V1 message') }],
...
for (
  const mutate of [
    value => {
      value.selection.participants[0].agentIdentity = { agentId: 'v2-only', revision: 'definition-1' }
    },
    value => {
      value.selection.activeRuns = []
    },
    value => {
      value.items[0].source = 'agent-loop'
    },
    value => {
      value.items[0].reactions = []
    },
  ]
) {
...
for (
  const mutate of [
    value => {
      value.selection.multiParticipant = false
    },
    value => {
      value.selection.participants[2].agentIdentity = { agentId: 'human', revision: 'definition-1' }
    },
    value => {
      value.selection.activeRuns[0].binding = { bindingId: 'binding-private', generation: 1 }
    },
    value => {
      value.selection.activeRuns[0].task = 'task-private'
    },
    value => {
      value.items[0].source = 'connector'
    },
    value => {
      delete value.items[0].source
    },
    value => {
      delete value.items[0].reactions
    },
    value => {
      value.items[0].reactions[0].html = '<b>x</b>'
    },
    value => {
      value.items[0].reactions[0].dom = 'node'
    },
    value => {
      value.items[0].reactions[0].callback = 'run'
    },
    value => {
      value.items[2].retry = { id: 'chatroom:retry-member' }
    },
    value => {
      value.items[2].body = { raw: true }
    },
    value => {
      value.items[2].trace = 'trace-private'
    },
  ]
) {
...
pushUpdate(itemWith(incremental, presenceSuccess.itemId, item => {
  item.state = 'creating'
}))
pushUpdate(itemWith(incremental, presenceSuccess.itemId, item => {
  item.state = 'joined'
...
pushUpdate(itemWith(incremental, presenceRetry.itemId, item => {
  item.state = 'failed'
  item.retryable = true
  item.diagnostic = text('Agent failed to join')
  item.retry = { id: 'chatroom:retry-member', arguments: { memberId: item.memberId, runId: item.runId } }
}))
pushUpdate(itemWith(incremental, presenceRetry.itemId, item => {
  item.state = 'creating'
...
pushUpdate(itemWith(incremental, message.itemId, item => {
  item.reactions[0].state = 'completed'
  item.reactions[1].state = 'failed'
}))
...
assert.deepEqual(
  normalizeWatermark(incremental),
  normalizeWatermark(convergedSnapshot),
  'incremental and replacement snapshots must converge',
)
const replayPage = {
  $schema: schemas.get('agent-conversation-shell-page.v2.schema.json').$id,
  contract: 'cordisx.agent-conversation-shell-page/v2',
  schemaVersion: 2,
  subscription,
  afterSequence: -1,
  phase: 'replay',
  updates: [{ kind: 'snapshot-replaced', sequence: 0, snapshot }],
  nextAfterSequence: 0,
  hasMore: true,
}
const livePage = {
  ...
  afterSequence: 0,
  phase: 'live',
  updates: [...incrementalUpdates, {
    kind: 'snapshot-replaced',
    sequence: streamSequence,
    snapshot: convergedSnapshot,
  }],
  nextAfterSequence: streamSequence,
  hasMore: false,
}
assert.ok(
  v['agent-conversation-shell-page.v2.schema.json'](replayPage),
  ajv.errorsText(v['agent-conversation-shell-page.v2.schema.json'].errors),
)
assert.ok(
  v['agent-conversation-shell-page.v2.schema.json'](livePage),
  ajv.errorsText(v['agent-conversation-shell-page.v2.schema.json'].errors),
)

function traceErrors(trace) {
  const errors = []
  if (!v['agent-conversation-shell-result.v2.schema.json'](trace.result)) {
    errors.push(ajv.errorsText(v['agent-conversation-shell-result.v2.schema.json'].errors))
  } else if (!isDeepStrictEqual(trace.result.subscription, trace.subscription)) {
    errors.push('accepted result subscription identity drift')
  }
  if (!v['agent-conversation-shell-command-context.v2.schema.json'](trace.context)) {
    errors.push(ajv.errorsText(v['agent-conversation-shell-command-context.v2.schema.json'].errors))
  } else if (
    !isDeepStrictEqual(trace.context.binding, trace.subscription.binding)
    || trace.context.generation !== trace.subscription.generation
  ) errors.push('command context binding or generation drift')
  ...
  for (const page of trace.pages) {
    if (!v['agent-conversation-shell-page.v2.schema.json'](page)) {
      errors.push(ajv.errorsText(v['agent-conversation-shell-page.v2.schema.json'].errors))
    }
    ...
    if (after < trace.subscription.snapshotSequence && page.phase !== 'replay') {
      errors.push('live precedes replay watermark')
    }
    for (const update of page.updates) {
      ...
      if (update.kind === 'snapshot-replaced') {
        ...
        if (
          replacementErrors.length === 0 && current.snapshotSequence > trace.subscription.snapshotSequence
          && !isDeepStrictEqual(normalizeWatermark(current), normalizeWatermark(update.snapshot))
        ) errors.push('snapshot replacement diverges from incremental state')
  ...
  if (!isDeepStrictEqual(normalizeWatermark(current), normalizeWatermark(trace.expectedFinal))) {
    errors.push('trace final snapshot drift')
  }
  return errors
}
...
const trace = {
  ...
  result,
  context,
  pages: [replayPage, livePage],
  expectedFinal: convergedSnapshot,
}
...
expectInvalidTransition({ ...presenceSuccess, state: 'ready' }, {
  ...presenceSuccess,
  state: 'failed',
  retryable: true,
})
expectInvalidTransition({ ...presenceSuccess, state: 'failed', retryable: false }, {
  ...presenceSuccess,
  state: 'creating',
})
...
expectInvalidTransition(message, {
  ...message,
  kind: 'status',
  label: text('cross-kind'),
  state: 'info',
  ariaLive: 'polite',
})
expectInvalidTransition(message, {
  ...message,
  reactions: [{ ...pendingEmoji, actorParticipantId: participant.participantId }, pendingSemantic],
})
expectInvalidTransition(message, {
  ...message,
  reactions: [{ ...pendingEmoji, value: { kind: 'semantic', token: 'changed' } }, pendingSemantic],
})
expectInvalidTransition(message, { ...message, reactions: [pendingSemantic, pendingEmoji] })
expectInvalidTransition(message, {
  ...message,
  reactions: [...message.reactions, { ...pendingEmoji, reactionId: 'reaction-duplicate-chip' }],
})
expectInvalidTransition({ ...message, reactions: [{ ...pendingEmoji, state: 'completed' }, pendingSemantic] }, message)
expectInvalidTransition({ ...message, reactions: [pendingEmoji, { ...pendingSemantic, state: 'failed' }] }, {
  ...message,
  reactions: [pendingEmoji, { ...pendingSemantic, state: 'completed' }],
})
expectInvalidTransition(message, {
  ...message,
  reactions: [...message.reactions, {
    reactionId: 'unknown-terminal',
    actorParticipantId: participant.participantId,
    value: { kind: 'semantic', token: 'new' },
    state: 'completed',
  }],
})
expectInvalidTransition(message, {
  ...message,
  reactions: [{ ...pendingEmoji, actorParticipantId: 'missing-participant' }, pendingSemantic],
})
...
for (
  const update of [
    { kind: 'item-updated', sequence: 1, item: { ...message, itemId: 'missing-item' } },
    {
      kind: 'item-appended',
      sequence: 1,
      item: { ...acknowledgement, itemId: 'new-item', sequence: snapshot.items.length, messageId: message.messageId },
    },
  ]
...
for (
  const mutate of [
    value => {
      value.items[2].participantId = humanParticipant.participantId
    },
    value => {
      value.items[2].participantId = identitylessAgent.participantId
    },
    value => {
      value.selection.activeRuns[1].participantId = otherAgent.participantId
    },
    value => {
      value.items[3].participantId = otherAgent.participantId
    },
    value => {
      value.items[2].state = 'joined'
      value.selection.activeRuns = value.selection.activeRuns.filter(run => run.runId !== value.items[2].runId)
    },
    value => {
      value.items[3].state = 'ready'
      value.selection.activeRuns = value.selection.activeRuns.filter(run => run.runId !== value.items[3].runId)
    },
    value => {
      value.items[3].state = 'joined'
      value.selection.activeRuns[1].participantId = otherAgent.participantId
    },
  ]
) {
...
for (
  const mutate of [
    value => {
      value.result.subscription.generation = 'other'
    },
    value => {
      value.pages[1].subscription = { ...value.pages[1].subscription, subscriptionId: 'stale-subscription' }
    },
    value => {
      value.pages[1].updates.at(-1).snapshot.items[0].source = 'chatroom-acknowledgement'
    },
  ]
) {
...
const shellSchemas = [...schemas.entries()].filter(([name]) => name.startsWith('agent-conversation-shell')).map((
  [, schema],
) => schema)
for (
  const token of [
...
) assert.ok(!JSON.stringify(shellSchemas).toLowerCase().includes(token), `forbidden ${token}`)
console.log(
  'Agent conversation shell conformance: frozen v1 plus v2 identity, presence, reaction, URL, convergence, and stream checks passed',
)
