...
function traceErrors(trace) {
  ...
  for (const current of trace.pages) {
    ...
    for (const update of current.updates) { if (disposed) errors.push('update follows terminal disposal'); if (update.sequence !== sequence) errors.push('update sequence is not monotonic'); sequence += 1; if (update.kind === 'disposed') disposed = true }
