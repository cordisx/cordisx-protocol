...
assert.equal(frozenDigest.digest('hex'), '2401260b6bc11538fc43378834048eb41b330f0de780d58579df35a459efa084', 'manifest/package v1-v5 and Agent/Session types must remain byte-frozen')
...
function resolveRouteScope(declaration, registration, routes, activeRoute) {
  ...
  const route = routes.find(candidate => candidate.document.id === binding?.routeId && candidate.owner.source === registration.source && candidate.owner.pluginId === registration.pluginId)
  if (route === undefined) errors.push('route is absent or belongs to another owner')
  if (route !== undefined) errors.push(...schemaErrors('route.v2.schema.json', route.document).map(error => `route ${error}`))
  if (route?.document.path.includes('*')) errors.push('wildcard routes cannot mint Session authority')
  const pathParameters = route?.document.path.split('/').filter(segment => segment.startsWith(':')).map(segment => segment.slice(1)) ?? []
  if (pathParameters.filter(param => param === binding?.param).length !== 1) errors.push('route must declare :sessionId exactly once')
  if (activeRoute?.owner?.source !== registration.source || activeRoute?.owner?.pluginId !== registration.pluginId) errors.push('active route owner mismatch')
  ...
  if (typeof sessionId !== 'string' || sessionId.length < 1 || sessionId.length > 512 || sessionId.includes('*')) errors.push('active route has no exact valid SessionId')
...
assert.notDeepEqual(schemaErrors('plugin-manifest.v5.schema.json', { ...manifest, $schema: schemaId('plugin-manifest.v5.schema.json'), schemaVersion: 5 }), [], 'manifest v5 must reject approval route capabilities')
for (const capability of agentRuntimeCapabilities) {
  const exact = { ...manifest, capabilities: [{ name: capability, required: capability !== 'approvals.request', scope: { sessionIds: ['session-exact-1'] } }] }
  assert.deepEqual(semanticErrors(exact), [], `${capability} must support exact SessionId scope`)
  const dynamic = { ...manifest, capabilities: [{ name: capability, required: false, scope: { sessionIds: routeScope } }] }
  assert.deepEqual(semanticErrors(dynamic), [], `${capability} must support optional exact Host route scope`)
}
assert.deepEqual(semanticErrors({ ...manifest, capabilities: [{ name: 'agents.create', required: true, scope: {} }] }), [], 'non-approval Agent capabilities may retain an unscoped maximum declaration')
...
const routes = Object.freeze([{ owner: { source: registration.source, pluginId: registration.pluginId }, document: exactSessionRoute }])
const activeRoute = Object.freeze({ owner: registration, routeId: 'room-session-detail', params: { roomId: 'room-1', sessionId: 'session-exact-1' } })
...
assert.notDeepEqual(semanticErrors({ ...manifest, capabilities: [requestDeclaration, structuredClone(requestDeclaration)] }), [], 'duplicate request declarations must fail closed')

const unavailableCases = [
  { routes: [{ owner: { source: registration.source, pluginId: 'other' }, document: exactSessionRoute }], activeRoute },
  { routes: [{ owner: { source: registration.source, pluginId: registration.pluginId }, document: { ...exactSessionRoute, path: '/main/chatroom/:roomId' } }], activeRoute },
...
for (const candidate of unavailableCases) assert.equal(resolveRouteScope(requestDeclaration, registration, candidate.routes, candidate.activeRoute).status, 'unavailable')
...
const packageManifest = Object.freeze({
  ...
  dependencies: [],
  runtimeManifest: { path: './runtime.json', schema: schemaId('plugin-manifest.v6.schema.json'), digest: `sha256:${'1'.repeat(64)}` },
})
assert.deepEqual(schemaErrors('plugin-package.v6.schema.json', packageManifest), [])
assert.notDeepEqual(schemaErrors('plugin-package.v5.schema.json', { ...packageManifest, $schema: schemaId('plugin-package.v5.schema.json'), schemaVersion: 5 }), [], 'package v5 must reject runtime manifest v6')
assert.ok(packageManifest.compatibility.protocolSchemas.includes(packageManifest.runtimeManifest.schema), 'package compatibility must name runtime manifest v6 exactly')

const publicBytes = JSON.stringify([schemas.get(schemaId('plugin-manifest.v6.schema.json')), schemas.get(schemaId('plugin-package.v6.schema.json'))])
for (const forbidden of ['rawPath', 'wildcardGrant', 'ownerOverride', 'generationOverride', 'sessionSearch', 'permissionWriter']) assert.equal(publicBytes.includes(forbidden), false)
