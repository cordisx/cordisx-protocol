...
export function validatePublisherGrant(value) {
  ...
  if (value.kind === 'grant' || value.kind === 'renew') {
    ...
    if (!(notBefore <= refreshAfter && refreshAfter < expiresAt)) errors.push('refreshAfter must be inside the authorization interval')
  }
  if (value.kind === 'revoke' && Date.parse(payload.effectiveAt) < issuedAt) errors.push('revocation cannot predate issuance')
  if (value.kind === 'transfer') {
    if (!(Date.parse(payload.notBefore) < Date.parse(payload.expiresAt))) errors.push('transfer timing order is invalid')
    if (payload.fromDevicePublicKeyHash === payload.toDevicePublicKeyHash) errors.push('transfer target must differ from source device')
  }
  return errors
}
...
let failures = 0
const canonicalVector = JSON.parse(await readFile(path.join(root, 'test-vectors/publisher-grants/canonicalization.json'), 'utf8'))
