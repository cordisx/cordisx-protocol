...
function upgradeWire(value) {
  ...
  const match = typeof next.$schema === 'string' ? next.$schema.match(/\/schemas\/(agent-loop-[a-z-]+)\.v[23]\.schema\.json$/) : undefined
...
function exchangeErrors(command, result, events = []) {
  ...
  if (isIntroduction && result.authorization.capability !== 'turns.introduce') errors.push('self-introduction authorization capability drift')
  if (!isIntroduction && !isApproval && result.causation !== undefined) errors.push('legacy accepted result acquired causation')
  if (result.status !== 'accepted') {
    if (result.causation !== undefined) errors.push('non-accepted result acquired causation')
    return errors
  }
  if (isIntroduction) {
    if (result.causation?.operationId !== command.commandId) errors.push('accepted self-introduction result causation drift')
    ...
    if (result.turn === undefined || result.messageId === undefined || result.delivery === undefined) errors.push('self-introduction accepted identity is incomplete')
    if (command.type === 'cancel-member-self-introduction' && result.requestOperationId !== command.requestOperationId) errors.push('self-introduction cancel target drift')
    for (const event of events) {
      if (!isDeepStrictEqual(event.binding, command.binding.binding)) errors.push('self-introduction event binding drift')
      if (!isDeepStrictEqual(event.causation, result.causation)) errors.push('self-introduction event/result causation drift')
    ...
    if (command.type === 'request-member-self-introduction') {
      ...
      if (failure !== undefined) {
        ...
        if (typeof failure.lifecycle.failure.retryable !== 'boolean') errors.push('failed self-introduction omitted retryability')
      } else if (completed) {
      ...
      if (events.length !== 1 || events[0].type !== 'lifecycle' || events[0].lifecycle.phase !== 'turn.cancelled') errors.push('accepted cancellation did not emit exactly one cancellation lifecycle event')
    }
  } else if (command.type === 'create-or-bind') {
    if (result.binding === undefined || result.detailsUrl === undefined || result.delivery === undefined) errors.push('accepted create/bind lost its v3 result fields')
  } else if (command.type === 'send') {
    if (result.binding === undefined || result.turn === undefined || result.messageId === undefined || result.delivery === undefined) errors.push('accepted send lost its v3 result fields')
    ...
    if (result.binding === undefined || result.turn !== command.turn || result.approvalId !== command.approvalId || result.decision !== command.decision || result.delivery === undefined) errors.push('accepted approval result drift')
    ...
    for (const event of events) {
      ...
      if (event.turn !== result.turn || event.approval?.approvalId !== result.approvalId || event.approval?.outcome !== result.decision) errors.push('resolved approval tuple drift')
...
function approvalTraceErrors(command, result, event, pending) {
  ...
  if (event === undefined || event.type !== 'approval' || event.approval.state !== 'resolved') errors.push('accepted approval omitted its resolved event')
  else if (pending !== undefined) {
    if (!isDeepStrictEqual(event.binding, pending.binding)) errors.push('resolved approval escaped pending binding')
    if (event.turn !== pending.turn || event.approval.approvalId !== pending.approval.approvalId || event.approval.kind !== pending.approval.kind) errors.push('resolved approval drifted from exact pending tuple')
  }
  return errors
}
...
function createApprovalOwner(bindings, pendingEvents) {
  ...
  const approvals = new Map(pendingEvents.map(event => [approvalKey({ binding: event.binding }, event.turn, event.approval.approvalId), { pending: structuredClone(event), state: event.approval.state, available: true }]))
  ...
  function resultBase(command, status, extra = {}) {
    return {
      $schema: schemas.get('agent-loop-result.v4.schema.json').$id,
      contract: 'cordisx.agent-loop-result/v4', schemaVersion: 4, commandId: command.commandId,
  ...
  function decide(command, clientId) {
    ...
    if (remembered !== undefined) {
      ...
      return { result, event: structuredClone(remembered.event), errors: approvalTraceErrors(command, result, remembered.event, remembered.pending), sideEffectCount }
    }
    let result
    let event
    let pending
    ...
    if (authorization.state === 'denied') result = resultBase(command, 'denied')
    ...
    else if (!isDeepStrictEqual(known, command.binding)) result = resultBase(command, 'conflict', { code: 'binding-conflict' })
      ...
      if (approval === undefined) result = resultBase(command, 'conflict', { code: 'approval-conflict' })
        ...
        result = resultBase(command, 'accepted', {
          binding: structuredClone(command.binding), turn: command.turn, approvalId: command.approvalId, decision: command.decision,
        ...
        event = {
          ...
          contract: 'cordisx.agent-loop-event/v4', schemaVersion: 4, eventId: `event-resolved-${command.commandId}`,
          binding: structuredClone(command.binding.binding), sequence: approval.pending.sequence + 1,
          occurredAt: '2026-08-31T12:00:00.000Z', causation: structuredClone(result.causation), type: 'approval', turn: command.turn,
          approval: { approvalId: command.approvalId, kind: approval.pending.approval.kind, state: 'resolved', outcome: command.decision },
        }
      }
    }
    ledger.set(command.commandId, { command: structuredClone(command), result: structuredClone(result), event: structuredClone(event), pending: structuredClone(pending), clientId })
  ...
  return {
    ...
    setApprovalState(command, value) { approvals.get(approvalKey(command.binding, command.turn, command.approvalId)).state = value },
    setApprovalAvailable(command, value) { approvals.get(approvalKey(command.binding, command.turn, command.approvalId)).available = value },
  }
}
...
function createIntroductionOwner(bindings, registeredCommands, privateInputs) {
  ...
  function replay(command, clientId, remembered) {
    ...
    const result = structuredClone(remembered.result)
    if (result.status === 'accepted') result.delivery.disposition = remembered.clientId === clientId ? 'replayed' : 'reconciled'
    return { result, events: structuredClone(remembered.events), errors: exchangeErrors(command, result, remembered.events), generationCount, cancellationCount }
  }
  ...
  function preflight(command) {
    ...
    if (authoritative === undefined) return introResultBase(command, 'conflict', allowedIntroduction, { code: 'binding-conflict' })
    if (authoritative.state === 'closed') return introResultBase(command, 'unavailable', allowedIntroduction, { code: 'binding-closed' })
    if (!isDeepStrictEqual(authoritative, command.binding)) return introResultBase(command, 'conflict', allowedIntroduction, { code: 'binding-conflict' })
    if (providerState === 'replaced') return introResultBase(command, 'unavailable', allowedIntroduction, { code: 'provider-replaced' })
    if (forcedCode !== undefined) return introResultBase(command, 'unavailable', allowedIntroduction, { code: forcedCode })
    const association = associationByBinding.get(bindingKey(command.binding))
    if (association === undefined || association.participantId !== command.participantId || association.memberId !== command.memberId) return introResultBase(command, 'conflict', allowedIntroduction, { code: 'member-conflict' })
    if (association.runId !== command.runId) return introResultBase(command, 'conflict', allowedIntroduction, { code: 'run-conflict' })
  }
  ...
  function request(command, clientId, options = {}) {
    ...
    if (failed !== undefined) {
      ledger.set(command.commandId, { command: structuredClone(command), result: structuredClone(failed), events: [], clientId })
    ...
    generationCount += 1
    const events = [{ ...eventBase(command, turn, `event-${command.commandId}-started`, 1), type: 'lifecycle', lifecycle: { phase: 'turn.started' } }]
    let state = 'pending'
    if (!options.defer && options.fail) {
      events.push({ ...eventBase(command, turn, `event-${command.commandId}-failed`, 2), type: 'lifecycle', lifecycle: { phase: 'turn.failed', failure: { code: 'INTRODUCTION_GENERATION_FAILED', retryable: true } } })
      state = 'failed'
    } else if (!options.defer) {
      events.push({ ...eventBase(command, turn, `event-${command.commandId}-message`, 2), type: 'message', message: { messageId, role: 'assistant', purpose: 'member-self-introduction', content: [{ kind: 'text', text: providerInput.providerAuthoredMessage }] } })
      events.push({ ...eventBase(command, turn, `event-${command.commandId}-completed`, 3), type: 'lifecycle', lifecycle: { phase: 'turn.completed' } })
      state = 'completed'
    }
    introductions.set(command.commandId, { command: structuredClone(command), result: structuredClone(result), state })
    ledger.set(command.commandId, { command: structuredClone(command), result: structuredClone(result), events: structuredClone(events), clientId })
  ...
  function cancel(command, clientId) {
    ...
    let result
    let events = []
    if (introduction === undefined) result = introResultBase(command, 'unavailable', allowedIntroduction, { code: 'introduction-not-found' })
    else if (!['participantId', 'memberId', 'runId'].every(field => introduction.command[field] === command[field]) || !isDeepStrictEqual(introduction.command.binding, command.binding)) result = introResultBase(command, 'conflict', allowedIntroduction, { code: 'introduction-conflict' })
    else if (introduction.state === 'completed') result = introResultBase(command, 'conflict', allowedIntroduction, { code: 'introduction-completed' })
    else if (introduction.state === 'cancelled') result = introResultBase(command, 'conflict', allowedIntroduction, { code: 'introduction-cancelled' })
    else if (introduction.state !== 'pending') result = introResultBase(command, 'conflict', allowedIntroduction, { code: 'introduction-conflict' })
    else {
      ...
      events = [{ ...eventBase(command, result.turn, `event-${command.commandId}-cancelled`, 2), type: 'lifecycle', lifecycle: { phase: 'turn.cancelled' } }]
    }
    ledger.set(command.commandId, { command: structuredClone(command), result: structuredClone(result), events: structuredClone(events), clientId })
    return { result, events, errors: exchangeErrors(command, result, events), generationCount, cancellationCount }
  }
...
for (const page of completeV2.pages) {
  ...
  assert.ok(page.events.every(event => isDeepStrictEqual(event.binding, page.subscription.binding)), 'legacy subscription escaped its exact binding generation')
}
...
for (const [first, replay] of [[idempotentV2.exchanges[0], idempotentV2.exchanges[1]], [idempotentV2.exchanges[2], idempotentV2.exchanges[3]]]) {
...
assert.deepEqual(approvalTraceErrors(approvalScenario.command, approvalScenario.acceptedResult, approvalScenario.resolvedEvent, approvalScenario.pendingEvent), [])
...
assert.notDeepEqual(approvalTraceErrors(approvalScenario.command, wrongApprovalCausation, approvalScenario.resolvedEvent, approvalScenario.pendingEvent), [])
...
assert.notDeepEqual(approvalTraceErrors(approvalScenario.command, approvalScenario.acceptedResult, driftedApprovalEvent, approvalScenario.pendingEvent), [])
const kindDriftEvent = structuredClone(approvalScenario.resolvedEvent)
kindDriftEvent.approval.kind = 'file-change'
assert.notDeepEqual(approvalTraceErrors(approvalScenario.command, approvalScenario.acceptedResult, kindDriftEvent, approvalScenario.pendingEvent), [])

for (const [mutation, mutate] of [
  ['binding-id', command => { command.binding.binding.bindingId = 'other-binding' }],
  ['binding-generation', command => { command.binding.binding.generation += 1 }],
  ['binding-task', command => { command.binding.task = 'other-task' }],
  ['binding-definition', command => { command.binding.definition.revision = 'other-definition' }],
  ['consumer-binding-state', command => { command.binding.state = 'closed' }],
...
expectApprovalOutcome(approvalOwner.decide(divergentApprovalCommand, 'approval-client'), 'same-operation-different-decision')
assert.equal(approvalOwner.sideEffectCount(), 1)
...
for (const command of [...scenario.requestCommands, scenario.cancelCommand]) assert.equal(validateCommand(command), true, ajv.errorsText(validateCommand.errors))
...
for (const providerInput of scenario.providerPrivateInputs) {
  for (const privateValue of [providerInput.definition.introduction, providerInput.definition.personality, providerInput.definition.role, ...providerInput.definition.capabilities, providerInput.providerAuthoredMessage]) {
    assert.equal(commandWire.includes(privateValue), false, 'provider-private definition input leaked into the consumer command')
  }
}
...
const closedIntroductionOwner = createIntroductionOwner(closedIntroductionBindings, scenario.requestCommands, scenario.providerPrivateInputs)
...
for (const invalidCase of invalidVector.acceptedMutations) {
  if (invalidCase.target === 'request') {
    ...
    else assert.notDeepEqual(exchangeErrors(scenario.requestCommands[0], invalid, scenario.requestEvents[0]), [], invalidCase.name)
    ...
    if (invalidCase.expected === 'schema-invalid') assert.equal(validateResult(invalid), false, invalidCase.name)
    else assert.notDeepEqual(exchangeErrors(scenario.cancelCommand, invalid, [scenario.cancelEvent]), [], invalidCase.name)
  } else {
    const events = structuredClone(scenario.requestEvents[0])
    events[1].causation.operationId = 'other-operation'
    assert.notDeepEqual(exchangeErrors(scenario.requestCommands[0], scenario.acceptedRequestResults[0], events), [], invalidCase.name)
  }
}
...
for (const invalidCase of invalidVector.forbiddenCausationCases) {
  ...
  if (invalidCase.type === 'create-or-bind') invalid = structuredClone(completeV2.createResult)
    ...
    if (invalidCase.status === 'denied') invalid = introResultBase(command, 'denied', { capability: 'turns.introduce', state: 'denied', code: 'policy-denied' })
    else if (invalidCase.status === 'unavailable') invalid = introResultBase(command, 'unavailable', allowedIntroduction, { code: 'introduction-unavailable' })
    else invalid = introResultBase(command, 'conflict', allowedIntroduction, { code: 'operation-conflict' })
  }
  assert.equal(validateResult(invalid), true, `forbidden-causation baseline must be valid: ${invalidCase.type}/${invalidCase.status}`)
...
console.log('Agent Loop v4 conformance: v3 create/send compatibility plus exact approval and self-introduction result causation, replay, retry, cancellation, provider fencing, and multi-agent isolation passed')
