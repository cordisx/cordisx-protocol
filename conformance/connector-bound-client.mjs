...
const schemaNames = [
  ...
  'connector-bound-client-lifecycle.v1.schema.json',
]
...
const sameRegistration = (left, right) =>
  left?.registrationId === right?.registrationId && left?.connectorId === right?.connectorId
  && left?.generation === right?.generation
const capabilityByCall = {
...
function validateSurface(value) {
  ...
  if (
    errors.length === 0
    && JSON.stringify([...value.operations].sort()) !== JSON.stringify(['discover', 'dispose', 'execute', 'subscribe'])
  ) {
...
function validateCall(call, binding, context) {
  ...
  const grant = binding.authorizations.find(record =>
    record.capability === capability
    && (call.type === 'discover'
      ? record.target.kind === 'catalog'
      : record.target.kind === 'registration' && sameRegistration(record.target.registration, call.registration))
  )
...
function validateExecution(call, execution, context, errors) {
  ...
  if (
    execution?.kind !== 'run.stopped' || !sameRegistration(execution.binding?.registration, call.registration)
  ...
  if (
    issued === undefined || !sameRegistration(issued.registration, call.registration)
    || issued.conversation !== call.command.conversation
  ) {
...
function validateExchange(vector) {
  ...
  if (errors.length > 0) return errors
  if (vector.result.callId !== vector.call.callId || vector.result.type !== vector.call.type) {
    errors.push('bound result does not match call')
  }
  ...
  if (
    vector.result.status !== expectedStatus || vector.result.authorization.state !== decision
  ...
  if (vector.result.status === 'accepted' && vector.call.type === 'execute') {
    validateExecution(vector.call, vector.result.execution, vector.context, errors)
  }
  if (vector.result.status === 'accepted' && vector.call.type === 'subscribe') {
    if (
      !sameRegistration(vector.result.subscription?.registration, vector.call.registration)
      || vector.result.subscription?.afterSequence !== vector.call.afterSequence
    ) errors.push('subscription does not match bound call')
    if (vector.result.subscription?.snapshotSequence < vector.call.afterSequence) {
      errors.push('subscription snapshot precedes requested cursor')
    }
  }
  return errors
}
...
function validatePages(vector, errors) {
  ...
  if (
    subscription.afterSequence < subscription.snapshotSequence
    && (!Array.isArray(vector.pages) || vector.pages.length === 0)
  ) {
  ...
  for (let index = 0; index < (vector.pages?.length ?? 0); index += 1) {
    ...
    ) errors.push(`page[${index}] subscription mismatch`)
    ...
    for (const event of page.events) {
      if (!sameRegistration(event.registration, subscription.registration)) {
        errors.push(`page[${index}] event registration drift`)
      }
      ...
      if (page.phase === 'replay' && event.sequence > subscription.snapshotSequence) {
        errors.push(`page[${index}] replay event exceeds snapshot`)
      }
    ...
    if (page.phase === 'replay' && !page.hasMore && page.nextAfterSequence !== subscription.snapshotSequence) {
      errors.push(`page[${index}] replay completes before snapshot`)
    }
...
function validateSubscription(vector) {
  ...
  if (
    vector.result?.status === 'accepted' && vector.result?.type === 'subscribe'
    && vector.result.subscription !== undefined
  ) validatePages(vector, errors)
  return errors
}
...
function validateLifecycle(vector) {
  ...
  const active = vector.attempt?.kind === 'call'
    ...
    : vector.value.clientState === 'active'
      && vector.value.subscriptions.find(subscription => subscription.subscriptionId === vector.attempt.subscriptionId)
          ?.state === 'active'
  if (vector.attempt?.allowed !== active) {
    errors.push('lifecycle attempt does not match terminal client/subscription state')
  }
  return errors
}

const validatorsByCase = {
  surface: vector => validateSurface(vector.value),
  result: vector => validators.result(vector.value) ? [] : errorsOf(validators.result),
...
for (
  ...
  if (publicSchemas.toLowerCase().includes(forbidden.toLowerCase())) {
    console.error(`Bound Connector client schemas must not expose ${forbidden}`)
    failures += 1
  }
}
const boundCallSchema = JSON.stringify(schemas.get('connector-bound-client-call.v1.schema.json'))
if (
  boundCallSchema.includes('principal') || boundCallSchema.includes('userHandle')
  || boundCallSchema.includes('authorization')
) {
