...
expect(
  'closed catalog proof digest matches canonical key serialization',
  common.completeCoverageProof.properties.catalogDigest.const === catalogDigest,
)
expect(
  'complete proof tuple count matches catalog Cartesian product',
  common.completeCoverageProof.properties.tupleCount.const
    === common.semanticIconKey.enum.length * common.variant.enum.length * common.state.enum.length,
)
...
expect(
  'trust semantics remain distinct',
  common.semanticIconKey.enum.indexOf('trust.certified') !== common.semanticIconKey.enum.indexOf('trust.official'),
)
...
expect(
  'all Manager semantics are closed and present',
  managerSemanticKeys.every(key => common.semanticIconKey.enum.includes(key)),
)
expect('Manager semantics remain distinct', new Set(managerSemanticKeys).size === 13)
expect(
  'favorite active is a state rather than another key',
  !common.semanticIconKey.enum.includes('action.favorite-active') && common.state.enum.includes('selected'),
)
...
function providerBindingIsValid(value) {
  ...
  if (principal.kind === 'host') {
    return identity.providerId === 'builtin:reicon' && identity.namespace === 'reicon'
      && completeCoverageIsValid(value.coverage, value.providerGeneration, identity)
  }
  return identity.providerId === `plugin:${principal.pluginId}:${identity.namespace}`
    && coverageIsValid(value.coverage, value.providerGeneration, identity)
}

function completeCoverageIsValid(coverage, providerGeneration, identity) {
  return coverage.kind === 'complete' && coverage.proof.providerId === identity.providerId
    && coverage.proof.namespace === identity.namespace && coverage.proof.providerVersion === identity.providerVersion
    && coverage.proof.providerGeneration === providerGeneration && coverage.proof.rawDataExported === false
    && coverage.proof.keyCount === 64 && coverage.proof.tupleCount === 1536
}
...
function coverageIncludes(coverage, request) {
  ...
  return coverage.entries.some(entry =>
    entry.key === request.key && entry.variant === request.variant && entry.state === request.state
  )
}
...
function sameProvider(left, right) {
  return left.providerHandle === right.providerHandle && left.providerId === right.providerId
    && left.namespace === right.namespace && left.protocolVersion === right.protocolVersion
    && left.providerVersion === right.providerVersion && left.providerGeneration === right.providerGeneration
}

function registrationMatches(reference, registration) {
  return registration !== undefined && reference.providerHandle === registration.providerHandle
    && reference.providerId === registration.identity.providerId
    && reference.namespace === registration.identity.namespace
    && reference.protocolVersion === registration.identity.protocolVersion
    && reference.providerVersion === registration.identity.providerVersion
    && reference.providerGeneration === registration.providerGeneration
}
...
function selectionIsValid(value) {
  ...
  return value.requestedProviderHandle !== value.selectedProvider.providerHandle
    && sameProvider(value.selectedProvider, value.fallbackProvider)
}
...
function resolutionExchangeIsValid(context, result) {
  ...
  if (
    request.providerHandle !== registration.providerHandle
    || request.providerGeneration !== registration.providerGeneration
  ) return false
...
function lifecycleTransitionIsValid(context, result) {
  ...
  if (
    operation.requestId !== result.requestId || operation.profileId !== result.profileId
    || operation.hostGeneration !== result.hostGeneration
  ) return false
  if (operation.expectedProfileRevision !== selection.profileRevision || result.operation !== action.kind) return false
  const unchanged = result.profileRevision === selection.profileRevision
    && sameProvider(result.activeProvider, selection.selectedProvider)
  if (result.outcome === 'conflict' || result.outcome === 'rejected' || result.outcome === 'rollback-failed') {
    return unchanged
  }
  if (result.profileRevision !== selection.profileRevision + 1) return false
  if (action.kind === 'register') {
    return result.affectedProviderHandle === action.providerHandle
      && sameProvider(result.activeProvider, selection.selectedProvider)
  }
  if (action.kind === 'select') {
    const registration = registrations.find(value =>
      value.providerHandle === action.providerHandle && value.providerGeneration === action.providerGeneration
    )
    if (result.outcome === 'rolled-back') {
      return result.affectedProviderHandle === action.providerHandle
        && sameProvider(result.activeProvider, selection.selectedProvider)
    }
    return registration !== undefined && result.affectedProviderHandle === action.providerHandle
      && registrationMatches(result.activeProvider, registration)
  }
  if (action.kind === 'dispose') {
    const targetsSelected = action.providerHandle === selection.selectedProvider.providerHandle
      && action.providerGeneration === selection.selectedProvider.providerGeneration
    return !targetsSelected && result.affectedProviderHandle === action.providerHandle
      && result.disposedGeneration === action.providerGeneration
      && sameProvider(result.activeProvider, selection.selectedProvider)
  }
  return action.failedProviderHandle === selection.selectedProvider.providerHandle
    && action.failedGeneration === selection.selectedProvider.providerGeneration
    && action.restoreProviderHandle === selection.fallbackProvider.providerHandle
    && action.restoreGeneration === selection.fallbackProvider.providerGeneration
    && result.affectedProviderHandle === action.failedProviderHandle
    && sameProvider(result.activeProvider, selection.fallbackProvider)
}
...
function semanticIsValid(schemaName, value, context, semantic) {
  ...
  if (semantic === 'selection-registration-pins') {
    return selectionIsValid(value)
      && registrationMatches(
        value.defaultProvider,
        context.registrations.find(registration =>
          registration.providerHandle === value.defaultProvider.providerHandle
        ),
      ) && registrationMatches(
        value.fallbackProvider,
        context.registrations.find(registration =>
          registration.providerHandle === value.fallbackProvider.providerHandle
        ),
      ) && registrationMatches(
        value.selectedProvider,
        context.registrations.find(registration =>
          registration.providerHandle === value.selectedProvider.providerHandle
        ),
      )
  }
  ...
  if (schemaName === 'icon-theme-lifecycle-operation.v1.schema.json' && value.operation.kind === 'register') {
    return value.operation.identity.providerId
        === `plugin:${value.operation.principal.pluginId}:${value.operation.identity.namespace}`
      && coverageIsValid(value.operation.coverage, value.operation.providerGeneration, value.operation.identity)
  }
  return true
}
...
expect(
  'selection default exact generation and version resolve to registration',
  registrationMatches(valid.selection.defaultProvider, reicon),
)
expect('selection pins default, selected, and fallback to the profile revision', selectionIsValid(valid.selection))
expect(
  'selection fallback is the exact default generation and version',
  sameProvider(valid.selection.fallbackProvider, valid.selection.defaultProvider),
)
...
expect(
  'partial coverage produces an explicit miss',
  valid.results[1].outcome === 'missing' && valid.results[1].reason === 'not-covered',
)

const dispose = valid.operations.find(value => value.operation.kind === 'dispose').operation
expect(
  'dispose cannot target the selected exact generation',
  dispose.providerHandle !== valid.selection.selectedProvider.providerHandle
    || dispose.providerGeneration !== valid.selection.selectedProvider.providerGeneration,
)
const rollback = valid.operations.find(value => value.operation.kind === 'rollback').operation
expect(
  'rollback retires the selected failed generation',
  rollback.failedProviderHandle === valid.selection.selectedProvider.providerHandle
    && rollback.failedGeneration === valid.selection.selectedProvider.providerGeneration,
)
expect(
  'rollback restores the pinned fallback generation',
  rollback.restoreProviderHandle === valid.selection.fallbackProvider.providerHandle
    && rollback.restoreGeneration === valid.selection.fallbackProvider.providerGeneration,
)
...
for (const operation of valid.operations) {
  ...
  transitionContexts.set(operation.requestId, {
    selection: structuredClone(currentSelection),
    operation,
    registrations: valid.registrations,
  })
  expect(`${operation.operation.kind} has a correlated lifecycle result`, result !== undefined)
  expect(
    `${operation.operation.kind} transition preserves exact pins`,
    lifecycleTransitionIsValid({ selection: currentSelection, operation, registrations: valid.registrations }, result),
  )
...
const boundary = JSON.parse(
  await readFile(path.join(root, 'test-vectors/icon-theme/valid/normalized-vector.json'), 'utf8'),
)
const boundaryValidator = validators.get('icon-theme-resolution-result.v1.schema.json')
expect(
  'normalized command descriptor is valid public data',
  boundaryValidator(boundary.value),
  boundaryValidator.errors,
)
expect(
  'normalized command descriptor has no raw path or SVG payload',
  JSON.stringify(boundary.value).includes('"d"') === false && JSON.stringify(boundary.value).includes('<svg') === false,
)

const trustSemantics = JSON.parse(
  await readFile(path.join(root, 'test-vectors/icon-theme/valid/trust-semantics.json'), 'utf8'),
)
...
expect(
  'certified and official use distinct semantic keys',
  trustSemantics.certified.key === 'trust.certified' && trustSemantics.official.key === 'trust.official'
    && trustSemantics.certified.key !== trustSemantics.official.key,
)
expect(
  'trust keys carry no user-facing text or raw source identity',
  ['label', 'accessibleName', 'publisherId', 'providerId', 'sourceId'].every(field =>
    !(field in trustSemantics.certified) && !(field in trustSemantics.official)
  ),
)

const managerSemantics = JSON.parse(
  await readFile(path.join(root, 'test-vectors/icon-theme/valid/manager-semantics.json'), 'utf8'),
)
...
expect(
  'Manager valid fixture covers each new key exactly once',
  JSON.stringify(managerSemantics.requests.map(request => request.key)) === JSON.stringify(managerSemanticKeys),
)
expect(
  'Manager semantics use the existing variant and state model',
  managerSemantics.requests.every(request =>
    common.variant.enum.includes(request.variant) && common.state.enum.includes(request.state)
  ),
)
expect(
  'favorite active uses selected state',
  managerSemantics.requests.find(request => request.key === 'action.favorite')?.state === 'selected',
)
expect(
  'Manager keys carry no user-facing text or raw source identity',
  managerSemantics.requests.every(request =>
    ['label', 'accessibleName', 'publisherId', 'providerId', 'sourceId'].every(field => !(field in request))
  ),
)
...
function contextIsValid(context) {
  ...
  if (context?.selection !== undefined && context?.operation !== undefined) {
    ...
    return selectionValidator(context.selection) && selectionIsValid(context.selection)
      && operationValidator(context.operation)
  }
  if (context?.registration !== undefined && context?.request !== undefined) {
    ...
    const requestValidator = validators.get('icon-theme-resolution-request.v1.schema.json')
    return registrationValidator(context.registration) && providerBindingIsValid(context.registration)
      && requestValidator(context.request)
  }
  if (context?.registrations !== undefined) {
    const registrationValidator = validators.get('icon-theme-provider-registration.v1.schema.json')
    return context.registrations.every(registration =>
      registrationValidator(registration) && providerBindingIsValid(registration)
    )
  }
  return true
}
