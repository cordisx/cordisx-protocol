...
const definitionBase = {
  $schema: schema('agent-definition.v1.schema.json'), contract: 'cordisx.agent-definition/v1', schemaVersion: 1,
...
assert.deepEqual(errors('agent-acquire-request.v1.schema.json', { ...createRequest, options: { vendorPolicy: { mode: 'safe' } } }), [])
...
export function validateAgentSetup(value) {
  ...
  for (const [identity, definition] of byId) for (const parent of definition.extends ?? []) {
    if (!byId.has(key(parent))) found.push(`${identity} has a missing ancestor`)
  }
  const visiting = new Set()
  const visited = new Set()
  function visit(identity) {
    if (visiting.has(identity)) { found.push(`definition cycle at ${identity}`); return }
...
assert.ok(validateAgentSetup({ ...setup, definition: { agentId: 'missing', revision: '1' } }).includes('root definition is absent'))
assert.ok(validateAgentSetup({ ...setup, definitions: [definitionBase, { ...definitionRoot, extends: [definitionRoot.identity] }] }).some(value => value.includes('cycle')))
...
assert.ok(errors('agent-acquire-result.v1.schema.json', { ...resultBase, operation: 'resume', status: 'denied', code: 'setup-conflict' }).length > 0)
assert.ok(errors('agent-acquire-result.v1.schema.json', { ...accepted, handle: {} }).length > 0)
assert.ok(errors('agent-acquire-result.v1.schema.json', { ...accepted, details: { kind: 'host', ref: 'https://example.com' } }).length > 0)
assert.ok(errors('agent-acquire-result.v1.schema.json', { ...accepted, details: { kind: 'host', ref: 'detail-1', url: 'https://example.com' } }).length > 0)
...
assert.ok(errors('agent-acquire-request.v1.schema.json', { ...createRequest, principal: { pluginId: 'forged', generation: 1 } }).length > 0)
const permissionCommon = JSON.parse(await readFile(path.join(root, 'schemas', 'permission-common.v4.schema.json'), 'utf8'))
assert.deepEqual(permissionCommon.$defs.capability.enum.filter(value => /^(?:agents|sessions|approvals)\./u.test(value)), capabilities)

export function ownerHandleAvailability(handle, current) {
  if (handle.owner.pluginId !== current.owner.pluginId || handle.owner.generation !== current.owner.generation) return 'plugin-generation-replaced'
...
assert.equal(ownerHandleAvailability(ownerHandle, { ...ownerHandle, owner: { pluginId: 'chatroom', generation: 4 } }), 'plugin-generation-replaced')
...
export function validateAcquireExchange(request, result) {
  ...
  if (result.status === 'accepted') {
    ...
    if (request.type === 'resume' && result.sessionIdSource !== 'caller') found.push('resume SessionId is never Host-minted')
    if (request.type === 'create' && request.sessionId === undefined && result.sessionIdSource !== 'host') found.push('Host-minted create misreported authority')
    if (request.type === 'create' && request.sessionId !== undefined && (result.sessionId !== request.sessionId || result.sessionIdSource !== 'caller')) found.push('caller create SessionId mismatch')
...
export function validateResumeRace(exchanges) {
  const found = []
  const winners = exchanges.filter(entry => entry.result.status === 'accepted' && entry.result.disposition === 'resumed')
  ...
  for (const entry of exchanges) {
    ...
    const sameRetry = entry.request.mutationId === winner.request.mutationId && entry.owner.pluginId === winner.owner.pluginId && entry.owner.generation === winner.owner.generation
    if (sameRetry) {
      if (entry.result.status !== 'accepted' || entry.result.disposition !== 'replayed') found.push('same-owner same-mutation retry must replay')
...
const race = [
  ...
  { owner: { pluginId: 'chatroom', generation: 3 }, request: resumeRequest, result: { ...accepted, disposition: 'replayed' } },
  { owner: { pluginId: 'trace', generation: 1 }, request: { ...resumeRequest, mutationId: 'resume-2' }, result: { ...resultBase, operation: 'resume', mutationId: 'resume-2', status: 'conflict', code: 'agent-already-live' } },
]
...
assert.ok(errors('agent-admission.v1.schema.json', { ...admission, status: 'denied', code: 'agent-replaced' }).length > 0)
for (const forbidden of ['operationId', 'turn', 'assistant', 'result', 'terminal', 'causation']) {
  assert.ok(errors('agent-admission.v1.schema.json', { ...admission, [forbidden]: 'forged' }).length > 0, `${forbidden} leaked into admission`)
}
...
assert.deepEqual(errors('agent-message-cancellation-result.v1.schema.json', { ...pendingCancellation, status: 'conflict', code: 'already-claimed' }), [])
assert.ok(errors('agent-message-cancellation-result.v1.schema.json', { ...pendingCancellation, status: 'denied', code: 'already-claimed' }).length > 0)
const wholeCancel = {
  $schema: schema('agent-mutation-result.v1.schema.json'), contract: 'cordisx.agent-mutation-result/v1', schemaVersion: 1,
  operation: 'cancel', mutationId: 'cancel-1', status: 'accepted',
}
...
assert.deepEqual(errors('agent-live-event.v1.schema.json', { ...live, type: 'vendor/telemetry', data: { safe: true } }), [])
