...
for (const value of chain) {
  assert.ok(
    validator('session-event.v1.schema.json')(value),
    JSON.stringify(validator('session-event.v1.schema.json').errors),
  )
}
...
export function approvalAuthorityChainErrors({ contextEvent, askedEvent, decidedEvent, question: liveQuestion }) {
  const issues = []
  if (contextEvent.type !== 'approval/authority-bound' || contextEvent.ignorable !== true) {
    issues.push('authority context must be one ignorable extension event')
  }
  if (
    contextEvent.sessionId !== liveQuestion.requester.sessionId
    || liveQuestion.requester.agentId !== liveQuestion.requester.sessionId
  ) issues.push('requester must own the exact Session')
  if (liveQuestion.authority.agentId !== liveQuestion.authority.sessionId) {
    issues.push('authority AgentId must equal its SessionId')
  }
  if (
    contextEvent.data.approvalId !== liveQuestion.id || askedEvent.data.id !== liveQuestion.id
    || decidedEvent.data.id !== liveQuestion.id
  ) issues.push('approval id mismatch')
  if (contextEvent.seq >= askedEvent.seq || askedEvent.seq >= decidedEvent.seq) {
    issues.push('approval facts out of order')
  }
  if (JSON.stringify(contextEvent.data.requester) !== JSON.stringify(liveQuestion.requester.definition)) {
    issues.push('requester definition mismatch')
  }
  if (JSON.stringify(contextEvent.data.authority) !== JSON.stringify(liveQuestion.authority.definition)) {
    issues.push('authority definition mismatch')
  }
  if (
    contextEvent.data.reason.kind !== 'plain-text' || contextEvent.data.reason.text !== askedEvent.data.reason
    || askedEvent.data.reason !== liveQuestion.reason.text
  ) issues.push('reason mismatch')
  return issues
}
...
assert.notDeepEqual(
  approvalAuthorityChainErrors({ ...valid, askedEvent: { ...chain[1], data: { ...chain[1].data, id: 'other' } } }),
  [],
)
assert.notDeepEqual(
  approvalAuthorityChainErrors({ ...valid, contextEvent: { ...chain[0], sessionId: 'other-session' } }),
  [],
)
assert.notDeepEqual(approvalAuthorityChainErrors({ ...valid, decidedEvent: { ...chain[2], seq: 1 } }), [])
assert.notDeepEqual(
  approvalAuthorityChainErrors({
    ...valid,
    askedEvent: { ...chain[1], data: { ...chain[1].data, reason: 'generic scenario label' } },
  }),
  [],
)
assert.notDeepEqual(
  approvalAuthorityChainErrors({
    ...valid,
    question: { ...question, authority: { ...authority, definition: requesterDefinition } },
  }),
  [],
)

export function uniqueDurableChainErrors(events, sessionId, approvalId) {
  const contexts = events.filter(value =>
    value.sessionId === sessionId && value.type === 'approval/authority-bound' && value.data.approvalId === approvalId
  )
  const asked = events.filter(value =>
    value.sessionId === sessionId && value.type === 'approval/asked' && value.data.id === approvalId
  )
  const decided = events.filter(value =>
    value.sessionId === sessionId && value.type === 'approval/decided' && value.data.id === approvalId
  )
  if (contexts.length !== 1 || asked.length !== 1 || decided.length !== 1) {
    return ['approval requires one unique authority-bound, asked, and decided chain']
  }
...
assert.notDeepEqual(
  uniqueDurableChainErrors(chain.slice(1), requester.sessionId, context.approvalId),
  [],
  'missing authority context fails closed',
)
assert.notDeepEqual(
  uniqueDurableChainErrors([...chain, chain[0]], requester.sessionId, context.approvalId),
  [],
  'duplicate authority context fails closed',
)
assert.notDeepEqual(
  uniqueDurableChainErrors(chain, authority.sessionId, context.approvalId),
  [],
  'foreign Session lookup cannot recover the chain',
)
...
const bindingKey = binding =>
  [binding.agentId, binding.sessionId, binding.agentGeneration, binding.definition.agentId, binding.definition.revision]
    .join('\u0000')
answerers.set(bindingKey(authority), () => 'allowed-once')
assert.equal(answerers.get(bindingKey(authority))(question), 'allowed-once')
assert.equal(
  answerers.get(bindingKey({ ...authority, agentGeneration: 8 })),
  undefined,
  'replacement generation cannot inherit authority',
)
assert.equal(
  answerers.get(bindingKey({ ...authority, sessionId: requester.sessionId })),
  undefined,
  'requester Session cannot impersonate authority',
)
...
assert.equal(
  digest.digest('hex'),
  'ce114e456f6c5b9a540db2a6f5957e5f548c99ed48d78783adfd9ed4f43f53c8',
  'Approval v1 public bytes drifted',
)
