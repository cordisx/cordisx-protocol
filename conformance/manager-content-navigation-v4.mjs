...
assert.equal(frozenDigest.digest('hex'), '947815ea64ac2b050d3076008950c48b80dcf5860b2065c26d8094d24e44d0ff', 'navigation v1-v3, projection v1-v2, and reused plugin-config bytes must remain frozen')
...
function applyOperations(value, operations) {
  ...
  for (const operation of operations) {
    ...
    for (const segment of operation.path.slice(0, -1)) {
      if (target[segment] === null || typeof target[segment] !== 'object' || Array.isArray(target[segment])) target[segment] = {}
...
function validateSuite(suite) {
  ...
  errors.push(...schemaErrors(validator('manager-content-navigation.v4.schema.json'), suite.declaration).map(error => `declaration ${error}`))
  errors.push(...schemaErrors(validator('manager-content-projection.v3.schema.json'), suite.projection).map(error => `projection ${error}`))
  if (suite.route?.owner !== suite.owner || suite.page?.owner !== suite.owner) errors.push('route and page must share the declaration owner')
  if (suite.route?.document?.outlet !== 'manager.content' || suite.route?.document?.page !== suite.page?.document?.id) errors.push('declaration route must resolve to one same-owner manager.content page')
  ...
  if ((declarationBody === undefined) !== (projectionBody === undefined)) errors.push('Host config body declaration and projection presence must match')
  if (declarationBody?.kind === 'plugin-config-form' && projectionBody?.kind === 'plugin-config-form') {
    if (!ownsNamespace(suite.owner, declarationBody.namespace)) errors.push('body namespace must be owned by the declaring plugin')
    ...
    for (const field of defaults) {
      ...
      else if (!equal(declared.value, field.value)) errors.push(`missing-only default ${key} differs from the Config schema default`)
    }
    const binding = projectionBody.binding
    const descriptor = projectionBody.configuration
    if (binding.declarationId !== suite.declaration.id || binding.namespace !== declarationBody.namespace) errors.push('projection binding must match declaration id and namespace')
    if (binding.identity.pluginId !== suite.owner || !ownsNamespace(binding.identity.pluginId, binding.namespace)) errors.push('projection binding owner is invalid')
    if (!equal(binding.identity, descriptor.identity) || !equal(binding.scope, descriptor.scope) || binding.namespace !== descriptor.namespace) errors.push('projection binding and plugin configuration descriptor fences must match exactly')
    if (descriptor.schema?.kind !== 'schemastery') errors.push('Host config form requires one renderable Schemastery projection')
    if (projectionBody.draft.baseRevision !== descriptor.revision) errors.push('draft base revision must equal the current descriptor revision')
    for (const secret of descriptor.secrets ?? []) {
      if (hasPath(descriptor.value, secret.path) || hasPath(descriptor.user, secret.path) || hasPath(projectionBody.draft.value, secret.path)) errors.push(`secret path is exposed: ${secret.path.join('.')}`)
    }
  }
  ...
  for (const [index, command] of commands.entries()) {
    errors.push(...schemaErrors(validator('manager-content-config-command.v1.schema.json'), command).map(error => `commands[${index}] ${error}`))
    const result = results[index]
    errors.push(...schemaErrors(validator('manager-content-config-result.v1.schema.json'), result).map(error => `results[${index}] ${error}`))
    ...
    for (const field of ['commandId', 'binding', 'expectedRevision', 'operation']) if (!equal(result?.[field], command[field])) errors.push(`results[${index}] ${field} must echo the command exactly`)
    if (result?.status === 'validated' && command.operation !== 'draft.validate') errors.push(`results[${index}] validated is only valid for draft.validate`)
    if (result?.status === 'preserved' && command.operation !== 'defaults.materialize') errors.push(`results[${index}] preserved is only valid for defaults.materialize`)
    if (result?.code === 'saved' && command.operation !== 'draft.save') errors.push(`results[${index}] saved must correspond to draft.save`)
    if (result?.code === 'defaults-materialized' && command.operation !== 'defaults.materialize') errors.push(`results[${index}] defaults-materialized must correspond to defaults.materialize`)
  }
  ...
  if (declarationBody?.kind === 'plugin-config-form' && projectionBody?.configuration?.schema?.kind === 'schemastery') {
    ...
    for (const [index, command] of commands.entries()) {
      ...
      if (command.expectedRevision !== revision) {
        if (result?.status !== 'conflict' || result.currentRevision !== revision) errors.push(`commands[${index}] stale CAS must conflict without retry`)
        continue
      }
      if (command.operation === 'defaults.materialize') {
        ...
        if (changed) {
          value = next
          revision += 1
          if (result?.status !== 'applied' || result.code !== 'defaults-materialized' || result.revision !== revision) errors.push(`commands[${index}] must atomically materialize only missing defaults`)
        } else if (result?.status !== 'preserved' || result.revision !== revision) errors.push(`commands[${index}] must preserve existing values idempotently`)
      } else {
        ...
        if (command.operation === 'draft.validate') {
          if (revision !== command.expectedRevision || (valid ? result?.status !== 'validated' : result?.status !== 'rejected')) errors.push(`commands[${index}] draft validation result is invalid`)
        } else if (!valid) {
          if (result?.status !== 'rejected' || result.code !== 'validation-failed') errors.push(`commands[${index}] invalid save must be rejected`)
        } else {
          value = candidate
          revision += 1
          if (!['applied', 'staged'].includes(result?.status) || result.revision !== revision) errors.push(`commands[${index}] save must use the same revision ledger`)
        }
      }
    }
    const preexisting = { shortcutPolicy: 'mod-enter' }
    const preserved = structuredClone(preexisting)
    for (const field of declarationBody.defaultMaterialization?.fields ?? []) if (!hasPath(preserved, field.path)) throw new Error('valid vector must exercise an existing default field')
  ...
  for (const [pageIndex, page] of (suite.subscriptionPages ?? []).entries()) {
    errors.push(...schemaErrors(validator('manager-content-config-subscription-page.v1.schema.json'), page).map(error => `subscriptionPages[${pageIndex}] ${error}`))
    ...
    for (const update of page.updates ?? []) {
      ...
      if (update.kind === 'snapshot-replaced' && !equal(update.body.binding, binding)) errors.push('subscription snapshot binding must remain exact')
      if (update.kind === 'disposed') terminal = true
    }
  }
  errors.push(...schemaErrors(validator('manager-content-config-subscription-close.v1.schema.json'), suite.subscriptionClose).map(error => `subscriptionClose ${error}`))
  if (!equal(suite.subscriptionClose?.binding, binding) || suite.subscriptionClose?.subscriptionId !== subscription?.subscriptionId) errors.push('subscription close fence must remain exact')
...
const publicSchemas = JSON.stringify(schemaNames.filter(name => name.startsWith('manager-content-')).map(name => schemas.get(name)))
for (const forbidden of ['homeConfigPath', 'localPath', 'rendererCallback', 'container', 'electronBridge', 'pluginMount', 'rawPath']) assert.equal(publicSchemas.includes(forbidden), false, `public schema must not expose ${forbidden}`)
